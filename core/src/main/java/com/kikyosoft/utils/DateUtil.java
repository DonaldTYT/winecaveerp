...
public class DateUtil /* 250821 to be added implements FunctionInterface, VariableInterface */{
...
//    public static java.util.Date minDate = new java.util.Date(-2208988800000L);  //  1st Jan  1900
	...
	static long gmtOffsetMinisec = new GregorianCalendar().get(Calendar.ZONE_OFFSET)+ new GregorianCalendar().get(Calendar.DST_OFFSET);
	/*
...
//CoreLog.log("getDate() return "+getDate(args[0], args[1]));
	   CoreLog.log(""+getDate(toDateString(today(), "yyyy")+"/01/01"));
	   CoreLog.log("trace:today="+today());
	   CoreLog.log("trace:yearStart="+yearStart(today())); CoreLog.log("trace:yearEnd="+yearEnd(today()));
	   CoreLog.log("trace:quarterStart="+quarterStart(today()));
	   CoreLog.log("trace:quarterEnd="+quarterEnd(today()));
	   CoreLog.log("trace:monthStart="+monthStart(today()));
	   CoreLog.log("trace:monthEnd="+monthEnd(today()));
	   CoreLog.log("trace:weekStart="+weekStart(today()));
	   CoreLog.log("trace:weekEnd="+weekEnd(today()));
		CoreLog.log("time="+today().getTime());
		CoreLog.log("time="+toDateString(new java.util.Date(now().getTime()+((long) 360)*86400*1000), "www, dd mmm yyyy HH:MM:SS ")+"GMT");
		CoreLog.log("time="+now());
		//
		java.util.Date date = getDate("1970/01/01");
	   CoreLog.log("trace:999000 date="+toDateString(date, "yyy/mm/dd"));
		java.util.Date date1 = getDate("1900/01/01");
	   CoreLog.log("trace:999000 date1="+toDateString(date1, "yyy/mm/dd"));
		java.util.Date date2 = getDate("1900/01/02");
	   CoreLog.log("trace:999000 date2="+toDateString(date2, "yyy/mm/dd"));
		java.util.Date date3 = getDate("1900/01/02");
	   CoreLog.log("trace:999000 date3="+toDateString(date3, "yyy/mm/dd"));
		java.util.Date date1 = getDate("1970/01/01");
	   CoreLog.log("trace:999000 getTime()="+date1.getTime());
	   CoreLog.log("trace:999000 date1="+toDateString(date1, "yyy/mm/dd"));
		CoreLog.log("trace: date="+(new java.util.Date(2524492800620L)));
	   CoreLog.log(getDate(args[0]).toString());
	   CoreLog.log("dayInWeek()="+dayInWeek(getDate(args[0])));
	   CoreLog.log("weekStartOnMondy()="+weekStartOnMonday(getDate(args[0])));
	   CoreLog.log("weekNumber()="+weekNumber(getDate(args[0])));
	   CoreLog.log("nextDaySkipHolidayStupid()="+nextDaySkipHolidayStupid(getDate(args[0]), NumberUtil.parseInt(args[1])));
	   CoreLog.log("nextDaySkipHoliday()="+nextDaySkipHoliday(getDate(args[0]), NumberUtil.parseInt(args[1])));
	   CoreLog.log("getMonth()="+getMonth(today()));
	   CoreLog.log("prevMonthStart()="+prevMonthStart(today()));
	   CoreLog.log("prevMonthEnd()="+prevMonthEnd(today()));
	   CoreLog.log("nextMonthStart()="+toDateString(nextMonthStart(today()), "yyyy-mm-dd"));
	   CoreLog.log("nextMonthEnd()="+toDateString(nextMonthEnd(today()), "yyyy-mm-dd"));
	   CoreLog.log("nextweek()="+toDateString(nextweek(today()), "yyyy-mm-dd"));
		CoreLog.log("day="+toDateString(today(), "dd"));
		CoreLog.log("nextmonth()="+toDateString(nextmonth(getDate(args[0]), Integer.parseInt(args[1])), "yyyy-mm-dd"));
		CoreLog.log("nextyear()="+toDateString(nextyear(getDate(args[0]), Integer.parseInt(args[1])), "mm-dd"));
		//CoreLog.log("compareTo()="+getDate(args[1]).compareTo(getDate(args[0])));
		*/

		//CoreLog.log("time="+toDateString(new java.util.Date(now().getTime()+((long) 360)*86400*1000), "HH:MM:SS"));
		//CoreLog.log("st="+toDateString((java.util.Date) getDateRange("currentWeek").elementAt(0), "yyyy-mm-dd"));
		//CoreLog.log("st="+toDateString((java.util.Date) getDateRange("currentWeek").elementAt(1), "yyyy-mm-dd"));

		//CoreLog.log("time0="+toDateString(weekStartOnSunday(today()), "yyyy-mm-dd"));
		//CoreLog.log("time1="+toDateString(weekEndWithSaturday(today()), "yyyy-mm-dd"));
		//CoreLog.log("today = " + new java.util.Date());
		//CoreLog.log("dayBeginning = " + DateUtil.dayBeginning(new java.util.Date()));
	   //System.out.println("getDate()="+ getDate(args[0], "yyyy-mm-dd"));
	   // CoreLog.log("trace:weekStart="+weekStart(getDate(args[0], "yyyy-mm-dd")));
	   // CoreLog.log("trace:weekEnd="+weekEnd(getDate(args[0], "yyyy-mm-dd")));
		/*
	   CoreLog.log("trace:time="+getTime(
		...
		CoreLog.log("" + DateUtil.weekNumber(new Date((new java.util.Date().getTime()) + 86400000)));
		CoreLog.log("" + DateUtil.weekNumber(new Date((new java.util.Date().getTime()))));
   	CoreLog.log("" + weekStartOnSunday(getDate(args[0])));
   	CoreLog.log("" + weekNumber(getDate(args[0]), true));
		*/
      //CoreLog.log("" + getWeekStartFromWeekNumber(NumberUtil.parseInt(args[0]), NumberUtil.parseInt(args[1]), true));
    	//CoreLog.log("week number=" + weekNumber(getDate(args[0]), true));
//		CoreLog.log("week year=" + getWeekNumberYear(getDate(args[0]), true));
		LogUtil.log(zeroDate.toString());
		LogUtil.log(minDate.toString());
		LogUtil.log1("" + secToText(5));
		LogUtil.log1("" + secToText(59));
		LogUtil.log1("" + secToText(60));
		LogUtil.log1("" + secToText(65));
		LogUtil.log1("" + secToText(86400));
		LogUtil.log1("" + secToText(86401));
		LogUtil.log1("" + secToText(3605000));
		
		LogUtil.log1(""+timeToMin("1:00"));
		LogUtil.log1(""+timeToMin("01:00"));
		LogUtil.log1(""+timeToMin("00:01"));
		LogUtil.log1(""+timeToMin("00:02"));
		LogUtil.log1(""+timeToMin("10:999"));
		LogUtil.log1("" + getDateY4MD("2023/08/29"));
		LogUtil.log1("" + getDateY4MD("2023/08/28"));
		LogUtil.log1("" + getDateY4MD("2023/08/a3"));
		LogUtil.log1("" + getDateY4MD("20200101"));
		LogUtil.log1("" + getDateY4MD("20201231"));
		LogUtil.log1("" + isValid(getDateY4MD("2050/12/31")));
	}
...
//				return(DateUtil.informixToDate((int) d)) ;
			...
			*/
			...
			if (p_logException){
				//CoreLog.log(pex);
				LogUtil.log1("error:" + pex.getMessage());
			}
			return(null);
		}
	}
	...
	public static String dateDigtalToTimeStr(java.util.Date p_date, boolean p_longFmt) {
		...
		//CoreLog.log1("dateDigtalToTimeStr %s,%d,%d", p_date, getGmtOffset(), l);
		if (l < 0)
 ...
 */
	...
	public static Date nextTask(Date p_lastTask, int p_interval, int p_nextHour, int p_nextMin) {
		//CoreLog.log1("lastTask:%s interval:%d", p_lastTask, p_interval);
		...
		if (p_lastTask == null) {
			nextTask = curDate;
			p_lastTask = curDate;
			//CoreLog.log1("hr:%d min:%d interval:%d last:%s next:%s immediate:%s (no last)", p_nextHour, p_nextMin, p_interval, df.format(p_lastTask),df.format(nextTask), nextTask.getTime() <= curDate.getTime());
			return (nextTask);
		}
		...
		if (nextTask.getTime() >= p_lastTask.getTime()) {
			//CoreLog.log1("hr:%d min:%d interval:%d last:%s next:%s immediate:%s (no interval)", p_nextHour, p_nextMin, p_interval, df.format(p_lastTask),df.format(nextTask), nextTask.getTime() <= curDate.getTime());
			return (nextTask);
		}
		
		cal.add(Calendar.MILLISECOND, p_interval);
		nextTask = cal.getTime();
		//CoreLog.log1("hr:%d min:%d interval:%d last:%s next:%s immediate:%s (add interval)", p_nextHour, p_nextMin, p_interval, df.format(p_lastTask),df.format(nextTask), nextTask.getTime() <= curDate.getTime());
		return (nextTask);
