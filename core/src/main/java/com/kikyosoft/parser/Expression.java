...
import com.kikyosoft.rpccall.Strval;
...
public class Expression {
	...
	public String toString() {
		if(type != 0) {
			if(type == OPERENDTYPE_CONSTANT && ((Cell) loperend).getType() == com.kikyosoft.cell.Cell.VTYPE_STRING) {
//				return("'"+loperend.toString()+"'");
				return("'"+loperend.toString().replaceAll("'", "\\\\'")+"'");
			} else if(type == OPERENDTYPE_CONSTANT && ((Cell) loperend).getType() == com.kikyosoft.cell.Cell.VTYPE_DATETIME) {
				return("'"+loperend.toString()+"'");
			} else if(type == OPERENDTYPE_CONSTANT && ((Cell) loperend).getType() == com.kikyosoft.cell.Cell.VTYPE_DATE) {
				return("'"+loperend.toString()+"'");
	...
	Cell evalArithmetic(Cell cLeft,int operator,Cell cRight) throws CellException {
		...
		if(cLeft == null) d0 = 0; else {
			...
			if(cLeft.getType() == com.kikyosoft.cell.Cell.VTYPE_DOUBLE) isDouble = true;
		}
		if(cRight == null) d1 = 0; else {
			d1 = cRight.getDouble();
			if(Double.isNaN(d1)) return(new Cell(Double.NaN));
			if(!isDouble && cRight.getType() == com.kikyosoft.cell.Cell.VTYPE_DOUBLE) isDouble = true;
		}
		switch(operator) {
		case OPERATOR_PLUS  : {
								double dr = d0 + d1;
...
//			} else return(loperend.toString());
			if(type == OPERENDTYPE_CONSTANT) {
				...
				case com.kikyosoft.cell.Cell.VTYPE_DATE :
					...
					break;
				case com.kikyosoft.cell.Cell.VTYPE_DATETIME:
					p_argList.add( DateUtil.toSqlTimestamp(((Cell)loperend).getDate()));
					break;
				case com.kikyosoft.cell.Cell.VTYPE_STRING|0x100:
...
//				}
	...
	public int getDataType() {
		...
		case OPERENDTYPE_BOOLEAN : return(com.kikyosoft.cell.Cell.VTYPE_BOOLEAN);
		default : return(0);
		}
	}
