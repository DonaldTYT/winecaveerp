...
public class Condition {
	static public int COMPARE_MODE_IGNORECASE = 1;
	static public int COMPARE_MODE_STRICTTYPE  = 2;
	...
	static public final int COMPARE_OP_SINGLE = 21;
	...
	public Condition (Expression p_expr) {
		isPredicate = true;
		operator = COMPARE_OP_SINGLE;
		loperend = p_expr;
	}
	public Condition (int p_compareMode,Expression p_loperend, int p_operator, Expression p_roperend1,Expression p_roperend2) throws Exception {
		switch(p_operator) {
		...
		isPredicate = true;
		ignoreCase = (p_compareMode & COMPARE_MODE_IGNORECASE) != 0;
		strictType  = (p_compareMode & COMPARE_MODE_STRICTTYPE) != 0;
		operator = p_operator;
		loperend = p_loperend;
		roperend = new DualExpression(p_roperend1,p_roperend2);
	}
	public Condition (int p_compareMode,Expression p_loperend, int p_operator, Expression p_roperend) throws Exception {
		ignoreCase = (p_compareMode & COMPARE_MODE_IGNORECASE) != 0;
		strictType  = (p_compareMode & COMPARE_MODE_STRICTTYPE) != 0;
		switch(p_operator) {
		case COMPARE_OP_GT:
	...
	public Condition (int p_compareMode,Expression p_loperend, int p_operator, List p_arglist) throws Exception {
		ignoreCase = (p_compareMode & COMPARE_MODE_IGNORECASE) != 0;
		strictType  = (p_compareMode & COMPARE_MODE_STRICTTYPE) != 0;
		switch(p_operator) {
...
//	}
	public String toString() {
		if(!isPredicate) {
			...
			case COMPARE_OP_SINGLE:
					return(((Expression) loperend).toString());
			case COMPARE_OP_IN_ITEMLIST : 
			case COMPARE_OP_NOTIN_ITEMLIST : 
						String s = null;
						for(Object o : (List) roperend) {
	...
	public HashSet getViewHash(HashSet vSet) throws CellException {
		if(!isPredicate) {
			...
			if(roperend != null) {
				vSet = ((Condition) roperend).getViewHash(vSet);
			}
		} else {
			if(loperend instanceof Expression) {
				vSet = ((Expression) loperend).getViewHash(vSet);
			}
			if(roperend != null) {
				if(roperend instanceof Expression) {
					vSet = ((Expression) roperend).getViewHash(vSet);
				}
				if(roperend instanceof List) {
					for(Expression o : (List <Expression>) roperend) {
						vSet = ((Expression) o).getViewHash(vSet);
					}
				}
				if(roperend instanceof DualExpression) {
					vSet = ((DualExpression) roperend).exp1.getViewHash(vSet);
					vSet = ((DualExpression) roperend).exp2.getViewHash(vSet);
				}
			}
		}
		return(vSet);
	...
	public HashSet getVariableHash(HashSet vSet,boolean p_unIndexed) throws CellException {
		if(!isPredicate) {
			...
			if(roperend != null) {
				vSet = ((Condition) roperend).getVariableHash(vSet,p_unIndexed);
			}
		} else {
			if(loperend instanceof Expression) vSet = ((Expression) loperend).getVariableHash(vSet,p_unIndexed);
			if(roperend != null) {
				if(roperend instanceof Expression) {
					vSet = ((Expression) roperend).getVariableHash(vSet,p_unIndexed);
				}
				if(roperend instanceof List) {
					for(Expression o : (List <Expression>) roperend) {
						vSet = ((Expression) o).getVariableHash(vSet,p_unIndexed);
					}
				}
				if(roperend instanceof DualExpression) {
					vSet = ((DualExpression) roperend).exp1.getVariableHash(vSet,p_unIndexed);
					vSet = ((DualExpression) roperend).exp2.getVariableHash(vSet,p_unIndexed);
				}
			}
		}
		return(vSet);
	...
	public HashSet getFunctionHash(HashSet vSet) throws CellException {
		if(!isPredicate) {
			if(loperend != null) {
				vSet = ((Condition) loperend).getFunctionHash(vSet);
			}
			if(roperend != null) {
				vSet = ((Condition) roperend).getFunctionHash(vSet);
			}
		} else {
			if(loperend instanceof Expression) vSet = ((Expression) loperend).getFunctionHash(vSet);
			if(roperend != null) {
				if(roperend instanceof Expression) {
					vSet = ((Expression) roperend).getFunctionHash(vSet);
				}
				if(roperend instanceof List) {
					for(Expression o : (List <Expression>) roperend) {
						vSet = ((Expression) o).getFunctionHash(vSet);
					}
				}
				if(roperend instanceof DualExpression) {
					vSet = ((DualExpression) roperend).exp1.getFunctionHash(vSet);
					vSet = ((DualExpression) roperend).exp2.getFunctionHash(vSet);
				}
			}
		}
		return(vSet);
	...
	public boolean eval(Object p_recData) throws Exception {
		...
		if(!isPredicate) {
			...
			case COMPARE_OP_LE :  {
					...
					switch(lc.getType()) {
					case com.kikyosoft.cell.Cell.VTYPE_DOUBLE : {
							double ld = 0,rd = 0;
							ld = lc.getDouble();
							if(Double.isNaN(ld)) {
								return(false);
							...
							if(roperend != null) {
								Cell crd = (((Expression) roperend).eval(p_recData));
								if(strictType && (crd.getType() != com.kikyosoft.cell.Cell.VTYPE_DOUBLE && crd.getType() != com.kikyosoft.cell.Cell.VTYPE_INT)) {
									return(false);
								}
								rd = crd.getDouble();
							}
					...
					case com.kikyosoft.cell.Cell.VTYPE_STRING: {
							String ls = lc.getString().trim();
							String rs;
							if(roperend != null) {
								Cell cr = ((Expression) roperend).eval(p_recData);
								if(strictType && cr.getType() != com.kikyosoft.cell.Cell.VTYPE_STRING) {
									return(false);
								}
							...
							if(ignoreCase) {
							switch(operator) {
							case COMPARE_OP_GT : return(ls.compareToIgnoreCase(rs) > 0);
							case COMPARE_OP_GE : return(ls.compareToIgnoreCase(rs) >= 0);
							case COMPARE_OP_EQ : return(ls.equalsIgnoreCase(rs));
							case COMPARE_OP_NE : return(!ls.equalsIgnoreCase(rs));
							case COMPARE_OP_LT : return(ls.compareToIgnoreCase(rs) < 0);
							case COMPARE_OP_LE : return(ls.compareToIgnoreCase(rs) <= 0);
							}
							} else {
							switch(operator) {
							case COMPARE_OP_GT : 
					...
					case com.kikyosoft.cell.Cell.VTYPE_INT: {
							int ld = 0,rd = 0;
							ld = lc.getInt();
					...
					case com.kikyosoft.cell.Cell.VTYPE_BOOLEAN: {
							Cell rc;
							if(roperend != null) rc = (((Expression) roperend).eval(p_recData)); else return(false);
							boolean rb=false;
							if(rc.getType() == com.kikyosoft.cell.Cell.VTYPE_BOOLEAN) {
										rb = rc.getBoolean();
							} else if(rc.getType() == com.kikyosoft.cell.Cell.VTYPE_STRING) {
										rb = rc.getString().equals("Y");
							} else if(rc.getType() == com.kikyosoft.cell.Cell.VTYPE_INT) {
										rb = rc.getInt() == 1;
							} else if(rc.getType() == com.kikyosoft.cell.Cell.VTYPE_DOUBLE) {
										rb = rc.getDouble() == 1.0;
							}
							switch(operator) {
					...
					case com.kikyosoft.cell.Cell.VTYPE_DATE: {
							java.util.Date ld = lc.getDate();
							java.util.Date rd;
							if(ld.before(DateUtil.minDate)) ld = DateUtil.zeroDate;
							if(roperend != null) rd = (((Expression) roperend).eval(p_recData)).getDate(); else return(false);
							if(rd.before(DateUtil.minDate)) rd = DateUtil.zeroDate;
							switch(operator) {
			...
			case COMPARE_OP_SINGLE: {
					Cell c = ((Expression) loperend).eval(p_recData);
					return(c.getBoolean());
				}
	...
	boolean isNull(Object operend,Object p_recData) throws Exception{
		...
		if(lc.getType() == com.kikyosoft.cell.Cell.VTYPE_DOUBLE) {
...
//				CoreLog.log("matrix " + i + "x" + j + " " + row.get(j));	
			}
...
//								CoreLog.log("A" + ruleConditionSet.toString() + " <-> " + groupCondition.toString());
							} else if(ruleConditionSet.containsAll(groupCondition)) {
//								CoreLog.log("B" + ruleConditionSet.toString() + " <-> " + groupCondition.toString());
								ruleEliminated = true;
								tmpHashSet.add(groupCondition);
							} else {
//								CoreLog.log("C" + ruleConditionSet.toString() + " <-> " + groupCondition.toString());
								tmpHashSet.add(groupCondition);
							}
						} else {
							if(groupCondition.containsAll(ruleConditionSet)) {
//								CoreLog.log("D" + ruleConditionSet.toString() + " <-> " + groupCondition.toString());
								ruleEliminated = true;
								tmpHashSet.add(groupCondition);
							} else if(ruleConditionSet.containsAll(groupCondition)) {
//								CoreLog.log("E" + ruleConditionSet.toString() + " <-> " + groupCondition.toString());
							} else {
								tmpHashSet.add(groupCondition);
//								CoreLog.log("F" + ruleConditionSet.toString() + " <-> " + groupCondition.toString());
							}
						}
					}
...
//				CoreLog.log("After " + groupConditionSet.toString());
			}
			Vector<Condition> result = new Vector<Condition>();
...
//				CoreLog.log("HAHA factorized: " + groupConditionStr);
