   ...
   public final static int TYPE_VARSCALARARRAY_END = 7;
	...
	public Object evalFunction(String p_fname,Vector args) throws Exception
	{
		...
		if(p_fname.equals("if")) {
			//LogUtil.log("CellCollectin eval 'if' " + args.size());
			if(args.size() != 3) return(null);
			//LogUtil.log("args " + ((Boolean) args.get(0)).booleanValue() + " " + args.get(1) + " " + args.get(2));
		...
		if(p_fname.equals("toValue")) {
			...
			if( o instanceof String) {
				try {
					...
					LogUtil.log("toValue " + o.toString() + "failed, return 0.0" );
					return(0.0);
				}
			}
		}
		...
		if(p_fname.equals("eval")) {
			...
			try {
				...
				LogUtil.log("eval formula error");
				return(0.0);
			}
		}
		if(parent != null) 
...
//				Date d1 = DateUtil.nextday(d0, (int) delta) ;
			...
			default:
				//LogUtil.log1("function (%s) not defined", p_fname);
		}
		if(parent != null) 
			return(parent.evalFunction(p_fname,args));
		else
	...
	public Cell getCell(String p_cname)
	{
		Cell c = testCell(p_cname);
		if (c == null) 
		   LogUtil.log(new Exception("getCell " + p_cname + " got null"));
		return(c);
	}
	public Cell testCell(String p_cname)
	{
	   ...
	   Field fd = (Field) ((Hashtable) reflectedFieldHashHash.get(getClass())).get(p_cname);
		...
		/*
		LogUtil.log(new Exception("getCell " + p_cname + " got null"));
		LogUtil.log("getCell " + p_cname + " got null");
		*/
		return(null);
	}
	public Cell[] getCells(String p_cname, boolean p_fComplaint) {
	   ...
	   Field fd = (Field) ((Hashtable) reflectedFieldHashHash.get(getClass())).get(p_cname);
		...
		if (p_fComplaint)
		   LogUtil.log("getCells " + p_cname + " got null");
		return(null);
	}
	...
	public Cell getCellArray(String p_cname, int p_idx) {
	   ...
	   Field fd = (Field) ((Hashtable) reflectedFieldHashHash.get(getClass())).get(p_cname);
		...
		LogUtil.log("getCellArray " + p_cname + " got null");
		return(null);
		*/
	}
		      ...
		      String key = (String) vv.elementAt(k);
			   ...
			   for (int i=0; i<v.size(); i++) {
				  if(!(v.elementAt(i) instanceof CellCollection)){
					  LogUtil.log("ignore non cellcollection element");
					  continue;
				  }
	...
	private static synchronized void makeReflected(CellCollection p_instance) {
      ...
      for (int i=0; i<fds.length; i++) {
		   Field fd = fds[i];
            ...
            //LogUtil.logClass(p_instance, "makeReflected: reflecting got array " + fd.getName());
		      reflectedFieldHash.put(fd.getName(), fd);
			}
		   else if (cl.isAssignableFrom(fd.getType())) {
            //LogUtil.logClass(p_instance, "makeReflected: reflecting got " + fd.getName());
		      reflectedFieldHash.put(fd.getName(), fd);
			}
		}
		   ...
		   Field fd = fds[i];
			try {
			   Object obj = fd.get(this);
				if (obj != null) {
			      if (obj instanceof Cell) {
						//LogUtil.log("HAHA__ reflecting got " + fd.getName());
	   ...
	   CellCollectionBrowserInterface browser = new CellCollectionBrowserInterface() {
	      ...
	      public void gotCollection(String p_cellName, CellCollection p_collection) throws Exception {
			   if (history.get(p_collection) != null) {
				   LogUtil.logClass(this, "saveToVectorUtil(): gotCollection(): browsed already "+p_cellName);
				   return;
				}
	...
	*/
	...
	public static void main_xxx(String[] args) throws Exception {
		/*
	   CellCollection cc = new CellCollection();
	   LogUtil.log("cc.testCell() return "+cc.testCell("colnames"));
	   Cell[] colnames = new Cell[1];
		colnames[0] = new Cell(10);
		cc.addCell("colnames", colnames);
	   LogUtil.log("cc.testCell() return "+cc.testCell("colnames"));
	   LogUtil.log("cc.getCells() return "+cc.getCells("colnames"));
	   LogUtil.log("cc.getCells() return "+cc.getCells("abcde"));
      ...
      VectorUtil vu = cc.saveToVectorUtil("xxcc", new VectorUtil(), new Hashtable());
	   LogUtil.log("vu.toVector() = "+vu.toVector());
	   LogUtil.log("vu.toVector().size() = "+vu.toVector().size());

      CellCollection cc1 = new CellCollection();
      int idx = parseFromVector(cc1, vu.toVector(), 0);
		LogUtil.log("idx="+idx);
		LogUtil.log("cc1="+cc1.toXMLRecursive(new StringBuffer()).toString());
		*/
		CellCollection cc = new CellCollection();
		cc.putValue("v1", 1);
		cc.putValue("v2", 2);
	...
	public static void main(String[] args) throws Exception {
	   ...
	   LogUtil.log(args[0]+"="+cc.getCollectionListByPath(args[0]));
	}
	*/
	...
	{
		checkReflected();
//		LogUtil.log("clearCellTrigger start " + this);
		if (cellTable != null) {
		   for (Enumeration e=cellTable.keys(); e.hasMoreElements(); ) {
				String s = (String) e.nextElement();
//				LogUtil.log("clearCell a " + s);
				Object o = cellTable.get(s);
				if(o != null && o instanceof Cell) {
					Cell c = (Cell) o;
					if(c.getFormula() != null) {
//						LogUtil.log("clearCellFormula a " + c.getFormula().toString());
						c.setFormula(null);
					}
				}
				if(o != null && o instanceof Cell[]) {
...
//							LogUtil.log("clearCellFormula c " + c.getFormula().toString());
							c.setFormula(null);
						}
					}
				}
			...
			if(fd != null) {
				try {
					...
					if(o != null && o instanceof Cell) {
						Cell c = (Cell) o;
						if(c.getFormula() != null) {
//							LogUtil.log("clearCellFormula b " + c.getFormula().toString());
							c.setFormula(null);
						}
					}
					if(o != null && o instanceof Cell[]) {
						...
						for(int i = 0;i<ca.length;i++) {
							Cell c = ca[i];
							if(c != null && c.getFormula() != null) {
//								LogUtil.log("clearCellFormula d " + c.getFormula().toString());
								c.setFormula(null);
							}
						}
					}
...
//		LogUtil.log("clearCellTrigger end " + this);
	}
