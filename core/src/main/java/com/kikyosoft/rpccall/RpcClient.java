...
//       conn.setRpcClient(this);
  ...
  public RpcClient open() throws RpcException
  {
    ...
    try {
      ...
      if (debug) {
		LogUtil.logClass(this, "open(): "+hostName+":"+portNumber);
      }
...
//      rpcSocket.setSoTimeout(timeoutMSec);
	  if (debug) {
			LogUtil.logClass( this, "Rpc Connection Turn On Tcp_No_Delay " + rpcSocket.getSendBufferSize());
	  }
  ...
  public Value callSegmentWithException(String segname) throws Exception {
    try {
      ...
      return(conn.callSegment(segname,null,0));
    	...
    	//LogUtil.log("conn.isDebug()="+conn.isDebug());
		if (conn.isDebug())
		   LogUtil.log(e);
		throw(e);
    }
