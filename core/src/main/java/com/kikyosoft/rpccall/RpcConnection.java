...
public class RpcConnection implements Valuable, RpcConst, Runnable {
	private	StreamMultiplexer nextMultiplexer = null;
...
//	private Socket sock;
	...
	*/
	...
	private boolean noAckFlag = true;
	...
	public void setDefaultServletClassName(String p_classname) {
		LogUtil.logClass(this, p_classname);
	...
	private void chkJVMVersion() {
		...
		// if (debug) LogUtil.logClass(this, vmvendor);
		if (vmvendor.startsWith("Microsoft Corp.")
			 ...
			 * String("ISO-8859-1");
			 */
			// LogUtil.logClass(this, "set encoding to Big5");
			/*
	...
	public RpcConnection(RpcServletProvider r) {
		provider = r;
		chkJVMVersion();
		String s = System
				.getProperty("com.kyoko.rpccall.nomethodwalker");
		if (s == null) {
			nowalker = false;
		} else {
			nowalker = s.equals("Y");
		}
		// LogUtil.logClass(this,"nomethodwalker " + nowalker);
		if (nowalker == false) {
	...
	*/
	...
	public void setTokenSupport(boolean sw) {
		...
		LogUtil.log("set tokenSupport to " + sw);
	}
	...
	public void setByteStream(ByteStream p_bs) throws IOException {
		bs = p_bs;
		bs.setTimeout(timeoutMSec);
	}
	...
	public void setSocket(Socket s) {
		...
		try {
			...
			if (debug) {
				LogUtil.logClass( this, "Rpc Connection Turn On Tcp_No_Delay " + sock.getSendBufferSize());
			}
			// sock.setSendBufferSize(8192);

			bs = new BufferedByteStream(new SocketByteStream(sock),2048,8192);
		} catch (Exception e) {
			LogUtil.log(e);
		}
	}
	*/
	...
	private RpcServlet searchFeature(String servicename) {
		...
		if (servicename.equals(""))
			return (null);
		// LogUtil.logClass(this, "search Feature " + servicename + " " +
		// Thread.currentThread());
		RpcConnServlet rpcconnservlet = (RpcConnServlet) servletInUse
				.get(servicename);
		try {
			if (rpcconnservlet != null) {
				rpcconnservlet.getServlet().setConnection(this);
		...
		if (rpcservlet != null) {
			...
			LogUtil.logClass(this, "Provider return null servlet");
		}
		return (rpcservlet);
	}
	...
	final public void run() {
		if (debug)
			LogUtil.logClass(this, "Connection runs " + Thread.currentThread());
		try {
			/*
			 ...
			 */
			for (;;) {
				int cc = ByteStream.readByte(bs);
				if (cc != SOH) {
					LogUtil.logClass(this, "Protocol error cannot get SOH");
					// changed by lai Fri Aug 10 23:30:31 HKG 2001 return;
					break;
				}
				LogUtil.log("240519 got SOH");
				realRpcServer();
				if (exitFlag)
					break;
				if(nextMultiplexer != null) {
					LogUtil.log("Converting rpc server to Mpx");
					nextMultiplexer.start();
					LogUtil.log("mpx started accepting channel 1 as rpc server");
					ByteStream mbs = nextMultiplexer.accept(1);
					LogUtil.log("channel 1 returned, setting rpcserver byte stream to mpx channel1");
					bs = mbs;
					nextMultiplexer = null;
				}
			}
		} catch (java.io.InterruptedIOException ioe) {
			LogUtil.logClass(this, this + " timeout");
		} catch (Exception e) {
		}
		...
		if (rpcclient != null)
			...
			*/
		if (debug)
			LogUtil.logClass(this, "Connection Gone" + Thread.currentThread());
		return;
	}
	...
	private int matchdirlist(String fname) {
		LogUtil.log("matchdirlist skipped.");
		return (1);
		/*
	...
	public int putFile(InputStream input, int maxlen, String filename,
		...
		try {
			...
			ByteStream.writeByte(bs,SOH);
			ByteStream.writeByte(bs,RPCPUTFILEID);
			ByteStream.writeInt(bs,offset);
			byte[] buf;
			if (simToTran) {
			...
			ByteStream.writeShort(bs,buf.length);
			ByteStream.loopWrite(bs,buf, 0, buf.length);
			ByteStream.writeByte(bs,STX);
			bs.flush();
			bs.setTimeout(RPCACKTIMEOUT * 1000);
			cc = ByteStream.readByte(bs);
			// sock.setSoTimeout(0);
			bs.setTimeout(timeoutMSec);
			switch (cc) {
			case ACK:
			...
			case NAK:
				...
				if (v == null)
					return (-1);
				LogUtil.logClass(this, "get fail 1 " + v.toString());
				v = null;
				return (-1);
			}
		} catch (IOException ioe) {
			LogUtil.logClass(this, "get fail 2 " + ioe.getMessage());
			return (-1);
		}
		return (-1);
	}
	...
	public int putFile(DataInput dataInput, int maxlen, String filename,
		...
		try {
			...
			case NAK:
				...
				if (v == null)
					return (-1);
				LogUtil.logClass(this, "get fail 3 " + v.toString());
				v = null;
				return (-1);
			}
		} catch (IOException ioe) {
	...
	private int rpcserver_putfile(DataInput input, int maxlen, int maxrate) {
		...
		try {
			...
			for (; eof;) {
				...
				if (readcnt > 0) {
					writecnt += readcnt;
					ByteStream.writeInt(bs,readcnt);
					ByteStream.loopWrite(bs,putbuf, 0, readcnt);
				}
			}
			ByteStream.writeInt(bs,0);
			bs.flush();
			if (ByteStream.readByte(bs) != EOT)
				throw new RpcException("Cannot get EOT");
		} catch (IOException e) {
			LogUtil.logClass(this,
					"rpcserver_putfile I/O error " + e.getMessage());
			LogUtil.log(e);
			return (-8);
		} catch (RpcException e) {
			LogUtil.logClass(this, "rpcserver_putfile error " + e.getMessage());
			LogUtil.log(e);
			return (-10);
		}
		return (writecnt);
	...
	public int getFile(DataOutput dataOutput, int maxlen, String filename,
		...
		try {
			lastCallTime = new Date();
			ByteStream.writeByte(bs,SOH);
			ByteStream.writeByte(bs,RPCGETFILEID);
			ByteStream.writeInt(bs,offset);
			byte[] buf;
			if (simToTran) {
				buf = ChineseConvert.convertB2G(filename)
			...
			ByteStream.loopWrite(bs,buf, 0, buf.length);
			ByteStream.writeByte(bs,STX);
			ByteStream.writeInt(bs,maxlen);
			ByteStream.writeInt(bs,maxrate);
			bs.flush();
			bs.setTimeout(RPCACKTIMEOUT * 1000);
			cc = ByteStream.readByte(bs);
			// sock.setSoTimeout(0);
			...
			case NAK:
				...
				if (v == null)
					return (-1);
				LogUtil.logClass(this, "get fail 4 " + v.toString());
				v = null;
				return (-1);
			}
		} catch (IOException ioe) {
			LogUtil.logClass(this, "get fail 5 " + ioe.getMessage());
			return (-1);
		}
		return (-1);
	}
	...
	public int getFile(OutputStream out, int maxlen, String filename,
		...
		try {
			...
			case NAK:
				...
				if (v == null)
					return (-1);
				LogUtil.logClass(this, "get fail 6 " + v.toString());
				v = null;
				return (-1);
			}
		} catch (IOException ioe) {
			LogUtil.logClass(this, "get fail 7 " + ioe.getMessage());
			return (-1);
		}
		return (-1);
	}
	...
	private int rpcserver_getfile(DataOutput outputstream) {
		try {
			for (;;) {
				...
				for (; len > 0;) {
					int cc = bs.read(getbuf, 0, len);
					len -= cc;
					outputstream.write(getbuf, 0, cc);
				}
			}
			ByteStream.writeByte(bs,EOT);
			bs.flush();
		} catch (IOException e) {
			LogUtil.logClass(this, "rpcserver_getfile " + e.getMessage());
			LogUtil.log(e);
			return (-1);
		}
		return (0);
						...
						for (varArgStart = 0; varArgStart < j; varArgStart++) {
							if (ptype[varArgStart] != margclasses[varArgStart]) {
								...
								if (pname2.equals("java.lang.Integer")
									...
									LogUtil.logClass(this, "argument "
											+ varArgStart + " " + pname2);
									LogUtil.logClass(this,
	...
	private Object invokeServlet(RpcServlet serv, String methodname,
		...
		if (nowalker) {
			...
			try {
				...
				varArgStart = -1;
				LogUtil.log("checkVarArg 1");
				mymethod = checkVarArg(myclass, margclasses, methodname,
						margobjects);
				LogUtil.log("checkVarArg 2");
				if (mymethod == null) {
					LogUtil.logClass(this, "myclassname=" + myclass.getName());
					LogUtil.logClass(this, "methodname=" + methodname);
					for (int z = 0; z < margobjects.length; z++) {
						LogUtil.logClass(this, "margclasses[" + z + "]="
								+ margclasses[z]);
					}
			...
			if (walker == null) {
				...
				if (debug)
					LogUtil.log("start build " + methodFullName);
				walker = new MethodWalker(methods, methodname);
				if (debug)
					LogUtil.log("end build " + methodFullName);
				walkerHash.put(methodFullName, walker);
			}
			// if(debug) LogUtil.log("start search " + methodFullName);
			mymethod = walker.searchMethodTree(margobjects);
			// if(debug) LogUtil.log("end search " + methodFullName);
			if (mymethod == null) {
				LogUtil.logClass(this, "method=" + methodFullName);
				for (int z = 0; z < margobjects.length; z++) {
					LogUtil.logClass(this, "margclasses[" + z + "]="
							+ margclasses[z]);
				}
	...
	private int realRpcServer() {
		int cc;
		try {
			int rpccallid = ByteStream.readByte(bs);
			int rpcserial = ByteStream.readInt(bs);
			int offset;
			short len = (short) ByteStream.readShort(bs);
			if (len <= 0)
			...
			if (rpccallid == RPCPUTFILEID) {
				...
				try {
					...
					Strval v = new Strval("FAIL [" + callname + "] "
							+ e.getMessage());
					ByteStream.writeByte(bs,NAK);
					writeValue(v);
					bs.flush();
					return (0);
				}
				callname = null;
				ByteStream.writeByte(bs,ACK);
				bs.flush();
				if (outputstream != null) {
			...
			if (rpccallid == RPCGETFILEID || rpccallid == RPCGETFILEID2
				...
				Vector arglist = null;
				int maxlen = ByteStream.readInt(bs);
				int maxrate = ByteStream.readInt(bs);
				if (rpccallid == RPCGETFILEID2) {
					kbyte = ByteStream.readInt(bs);
					len = (short) ByteStream.readShort(bs);
					buf = new byte[len];
					offset = 0;
					for (; len > 0;) {
						int rlen;
					...
					int reserved = ByteStream.readInt(bs);
					int argcnt = ByteStream.readShort(bs);
					if (argcnt > 0)
						arglist = new Vector();
				...
				try {
					...
					if (rpccallid == RPCGETFILEIDWITHTIME) {
						LogUtil.log("sendFileTime true, sending DC2 and filetime");
						File f = new File(callname);
						ByteStream.writeByte(bs,DC2);
						ByteStream.writeInt(bs,(int) (f.lastModified() / 1000));
					}
					ByteStream.writeByte(bs,ACK);
					bs.flush();
			...
			short numarg = (short) ByteStream.readShort(bs);
			if (numarg < 0)
			...
			Value v = null;
			// LogUtil.logClass(this, "callname " + callname);
			...
			if (tokencnt == 1) {
				...
				// LogUtil.logClass(this, "Method " + methodname);
...
//				return (-1);
				...
				arglist = null;
				if (rpccallid != RPCCALLSEGMENTID_NORETURN) {
					LogUtil.log("rpc " + callname + " not found");
					ByteStream.writeInt(bs,rpcserial);
					if (v == null)
						v = new Longval(-1);
					writeValue(v);
					ByteStream.writeByte(bs,EOT);
			...
			try {
				...
				for (i = 0; i < numarg; i++) {
					...
					case ISDVALUE:
						...
						// value properly.
						// LogUtil.log("HAHA in rpcserverconnection rpcservercallback "
						// + dval);
						if (dval.toInt() <= 0) {
							margobjects[i] = null;
						} else {
							...
							margobjects[i] = tdate;
							// LogUtil.log("HAHA in rpcserverconnection rpcservercallback jd = "
							// + tdate);
						}
						break;
					}
				...
				if (nowalker) {
					...
					try {
						...
						LogUtil.log("checkVarArg 2");
						if (mymethod == null) {
							LogUtil.logClass(this, "myclassname=" + myclassname);
							LogUtil.logClass(this, "methodname=" + methodname);
							for (int z = 0; z < margobjects.length; z++) {
								LogUtil.logClass(this, "margclasses[" + z + "]="
										+ margclasses[z]);
					...
					if (debug)
						LogUtil.log("start search " + methodFullName);
					mymethod = walker.searchMethodTree(margobjects);
					if (debug)
						LogUtil.log("end search " + methodFullName);
					if (mymethod == null) {
						if (debug) {
							LogUtil.logClass(this, "method=" + methodFullName);
							for (int z = 0; z < margobjects.length; z++) {
					...
					}
					{
						 ...
						 * (IOException ex) { LogUtil.logClass(this,
						 * "exteption trapped inside invoke"); LogUtil.log(ex);
						 * return(-1); }
						 */
					}
				}
				...
				if (debug)
					LogUtil.logClass(this, "return type " + rettype);
				if (retClass
					...
					v = new Dateval(
							com.kikyosoft.utils.DateUtil
									.dateToInformix(dateret));

				} else if (retClass.isAssignableFrom(Class
						.forName("com.kyoko.rpccall.Value"))) {
					v = (Value) retobj;
				} else {
					if (retobj != null)
			...
			LogUtil.logClass(this, "Error in rpc protocol " + e.getMessage());
			LogUtil.log(e);
			cc = -1;
		}
		return (cc);
	...
	public void stopService() {
		...
		try {
			...
			LogUtil.logClass(this, "stop service fail");
		}
		provider = null;
		servletInUse = null;
		bs = null;
	...
	private boolean writeValue(Value val) throws IOException {
		...
		case ISTVALUE:
			...
			ByteStream.writeShort(bs,V_TOKEN);
			ByteStream.writeInt(bs,tokenval.toInt());
			break;
		case ISTSVALUE:
			TokenStringVal tsval = (TokenStringVal) val;
			ByteStream.writeShort(bs,V_STRING_WITH_TOKEN);
			ByteStream.writeInt(bs,tsval.toInt());
			byte[] mybuf;
			if (simToTran) {
			...
			ByteStream.writeShort(bs,mybuf.length);
			ByteStream.loopWrite(bs,mybuf, 0, mybuf.length);
			break;
		case ISSVALUE:
			...
			if (strval.getEncoding() != null) {
				byte[] buf2;
				{
					...
					for (int i = 0; i < cr.length; i++) {
						int ci;
						ci = cr[i];
						LogUtil.log("UTF8 convert C " + i + " " + ci);
					}
				}
				/*
				 * if(simToTran) { buf2 =
				...
				if (strval.getEncoding().equals("SJIS"))
					...
					buf[1] = 'u';
				LogUtil.log("HAHA_UTF8 A " + buf2.length);
				for (int i = 0; i < buf2.length; i++) {
					LogUtil.log("HAHA_UTF8 B " + i + " " + buf2[i]);
					buf[i + 2] = buf2[i];
				}
			} else {
				if (simToTran) {
			...
			if (buf.length > 32767) {
				ByteStream.writeShort(bs,V_LONGSTR);
				ByteStream.writeInt(bs,buf.length);
			} else {
				ByteStream.writeShort(bs,V_STRING);
				ByteStream.writeShort(bs,buf.length);
			}
			ByteStream.loopWrite(bs,buf, 0, buf.length);
			break;
		case ISRVALUE:
			ByteStream.writeShort(bs,V_FLOAT);
			long longbits = Double.doubleToLongBits(val.toDouble());
			for (int i = 0; i < 8; i++) {
				ByteStream.writeByte(bs,(byte) (longbits & 0xff));
				longbits >>= 8;
			}
			/*
			...
			break;
		case ISIVALUE:
			ByteStream.writeShort(bs,V_LONG);
			ByteStream.writeInt(bs,val.toInt());
			break;
		case ISDVALUE:
			ByteStream.writeShort(bs,V_DATE);
			ByteStream.writeInt(bs,val.toInt());
			break;
		case ISBUFVALUE:
			byte ba[] = ((Bufval) val).getBuffer();
			ByteStream.writeShort(bs,V_BYTEARR);
			// Fri Aug 4 16:03:48 HKG 2006
			// use short instead of int , compatible with perf bytearr
			// os.writeInt(ba.length);
			ByteStream.writeShort(bs,(short) ba.length);
			for (int i = 0; i < ba.length; i++)
				ByteStream.writeByte(bs,ba[i]);
			break;
		default:
			return (false);
		}
	...
	private Value readValue() throws IOException {
		...
		int offset;
		int len;
		cc = ByteStream.readShort(bs);
		// LogUtil.logClass(this, "readvalue type = " + cc);
		switch (cc) {
		case V_NULL:
			short t = (short) ByteStream.readShort(bs);
			return (new Nulval(t));
		case V_BYTEARR:
			len = ByteStream.readShort(bs);
			if (len >= 0) {
				byte buf[] = new byte[len];
				offset = 0;
				for (; len > 0;) {
		...
		case V_LONGSTR:
			...
			if (len > 0) {
				...
				if (offset >= 2 && buf[0] == -1 /* && buf[1] == 'u' */) {
					switch (buf[1]) {
					case 'j':
						LogUtil.log("SJIS convert");
						return (new Strval(new String(buf, 2, offset - 2,
								"SJIS")));
					default:
						LogUtil.log("UTF8 convert");
						/*
						 ...
						 */
						{
							for (int i = 0; i < buf.length; i++) {
								LogUtil.log("UTF8 convert A " + i + " " + buf[i]);
							}
							...
							for (int i = 0; i < cr.length; i++) {
								int ci;
								ci = cr[i];
								LogUtil.log("UTF8 convert B " + i + " " + ci);
							}
		...
		case V_LONG:
		case V_INTEGER:
			return (new Longval(ByteStream.readInt(bs)));
		case V_DATE:
			return (new Dateval(ByteStream.readInt(bs)));
		case V_FLOAT:
			long longbits = 0;
			for (int i = 0; i < 64; i += 8) {
				 ...
				 */
				long tlong = ByteStream.readUnsignedByte(bs);
				tlong <<= i;
				longbits |= tlong;
			}
	...
	// throws RpcException, ClassNotFoundException
	{
		if (debug) {
			if (argcnt == 0)
				LogUtil.logClass(this, "RpcConnection callsegment " + returnFlag
						+ " " + segname + " " + argcnt);
		}
		try {
			int i = 0;
			...
			if (argcnt > 0) {
				...
				while (enum0.hasMoreElements()) {
					...
					*/
					...
					if (aClass.isAssignableFrom(Class
							...
							if (rtoken != null) {
								...
								if (idx == 0) {
									...
									vlist.addElement(new TokenStringVal(strret,
											tokenidx));
									// LogUtil.log("init new Token arg " +
									// tokenidx + " " + strret);
								} else {
									vlist.addElement(new TokenVal(idx));
									// LogUtil.log("use Token arg " + idx);
								}
							} else {
						...
						vlist.addElement(new Longval(intret.intValue()));
						LogUtil.logClass(this, "not support long type"); // andrew190323:
																		// server
																		// side
																		// will
																		// treat
						...
						// + 2209075200000L;
						// int l = (int) (ll / 86400000);
						int l = com.kikyosoft.utils.DateUtil
								.dateToInformix(dateret);

						// LogUtil.log("HAHA in RpcServerConnection date getTime "
						// + dateret.getTime());
						// LogUtil.log("HAHA in RpcServerConnection date getGmtOffset "
						// + DateUtil.getGmtOffset());
						// LogUtil.log("HAHA in RpcServerConnection date argument "
						// + dateret + " -> " + l);
				...
				if (debug) {
					LogUtil.logClass(this, sb.append("]").toString());
				}
			} else {
				vlist = new Vector();
			}
...
//			LogUtil.log("before call remote server");
			Value v =
					(callRemoteServer(segname,
					returnFlag ? (noAckFlag ? RPCCALLSEGMENTID_NOACK
							: RPCCALLSEGMENTID) : RPCCALLSEGMENTID_NORETURN,
					vlist.elements(), vlist.size()));
// 			LogUtil.log("after call remote server");
			return(v);
		} catch (Exception e) {
			LogUtil.log("call fail: " +e.getMessage());
			if (debug) {
				LogUtil.log(e);
			}
			throw (e);
	...
	private Value callRemoteServer(String segname, int callid,
		...
		try {
			...
			lastCallTime = new Date();
			ByteStream.writeByte(bs,SOH);
			ByteStream.writeByte(bs,callid);
			ByteStream.writeInt(bs,tmpserial);
			if (tokenSupport) {
				...
				if (rtoken != null) {
					...
					if (idx == 0) {
						tokenidx++;
						rtoken.setIndex(tokenidx);
						byte[] ba = segname.getBytes();
						ByteStream.writeShort(bs,ba.length);
						ByteStream.loopWrite(bs,ba,0,ba.length);
						/*
						ByteStream.writeShort(bs,segname.length());
						ByteStream.writeBytes(bs,segname);
						*/
						ByteStream.writeByte(bs,DC1);
						ByteStream.writeInt(bs,tokenidx);
						// LogUtil.log("init new Token seg " + tokenidx + " " +
						// segname);
					} else {
						ByteStream.writeShort(bs,0);
						ByteStream.writeInt(bs,idx);
						ByteStream.writeByte(bs,STX);
						// LogUtil.log("use Token seg " + idx);
					}
				} else {
					byte[] ba = segname.getBytes();
					ByteStream.writeShort(bs,ba.length);
			 ...
			 * os.writeByte(STX); }
			 */
			ByteStream.writeShort(bs,argcnt);
			while (enum0.hasMoreElements()) {
				writeValue((Value) enum0.nextElement());
			}
			ByteStream.writeByte(bs,ETX);
			...
			if (debug)
				LogUtil.logClass(this, "Couldn't get I/O for the connection");
			if (debug)
				LogUtil.log(e);
			throw (e);
		} catch (RpcException e) {
	...
	private int readSerial() throws IOException {
		...
		t = 0;
		for (int i = 0; i < 3; i++) {
			cc = ByteStream.readUnsignedByte(bs);
			t = t << 8;
			t |= cc;
	...
	public void setDebug(boolean p_flag) {
		debug = p_flag;
		if (debug) {
			LogUtil.log("Server Connection setDebug " + p_flag);
		}
	}
	...
	*/
	...
	public int putFileWithCallback(InputStream input, int maxlen,
		...
		try {
			...
			case NAK:
				...
				if (v == null)
					return (-1);
				LogUtil.logClass(this, "get fail 8 " + v.toString());
				v = null;
				return (-1);
			}
		} catch (IOException ioe) {
			LogUtil.logClass(this, "get fail 9 " + ioe.getMessage());
			return (-1);
		}
		return (-1);
	}
	...
	public int putFileWithCallback(DataInput dataInput, int maxlen,
		...
		try {
			...
			case NAK:
				...
				if (v == null)
					return (-1);
				LogUtil.logClass(this, "get fail 10 " + v.toString());
				v = null;
				return (-1);
			}
		} catch (IOException ioe) {
	...
	private void fileProgress(String p_methodname, Vector arglist) {
		...
		try {
			...
			if (serv == null) {
				LogUtil.log("fileProgress fail 1 " + p_methodname + " "
						+ myclassname);
				return;
			}
			...
			LogUtil.log("fileProgress fail 2 " + p_methodname + " "
					+ myclassname);
	...
	private void writeGetFileExtendedParameter(int kbyte, String mode,
			...
			IOException {
		try {
			ByteStream.writeInt(bs,kbyte);
			// writeValue(new Strval(callbackname));
			byte[] ba = callbackname.getBytes();
			ByteStream.writeShort(bs,ba.length);
			ByteStream.loopWrite(bs,ba,0,ba.length);
			/*
			ByteStream.writeShort(bs,callbackname.length());
			ByteStream.writeBytes(bs,callbackname);
			*/
			ByteStream.writeInt(bs,0); /* reserved */
			int argcnt = (arglist == null ? 0 : arglist.size());
			ByteStream.writeShort(bs,argcnt);
			for (int i = 0; i < argcnt; i++) {
	...
	public int getFileWithCallback(DataOutput dataOutput, int maxlen,
		...
		try {
			lastCallTime = new Date();
			ByteStream.writeByte(bs,SOH);
			ByteStream.writeByte(bs,kbyte > 0 ? RPCGETFILEID2 : RPCGETFILEID);
			ByteStream.writeInt(bs,offset);
			byte[] buf;
			if (simToTran) {
				buf = ChineseConvert.convertB2G(filename)
			...
			case NAK:
				...
				if (v == null)
					return (-1);
				LogUtil.logClass(this, "get fail 11 " + v.toString());
				v = null;
				return (-1);
			}
		} catch (IOException ioe) {
			LogUtil.logClass(this, "get fail 12 " + ioe.getMessage());
			return (-1);
		}
		return (-1);
	}
	...
	public int getFileWithCallback(OutputStream out, int maxlen,
		...
		try {
			...
			case NAK:
				...
				if (v == null)
					return (-1);
				LogUtil.logClass(this, "get fail 13 " + v.toString());
				v = null;
				return (-1);
			}
		} catch (IOException ioe) {
			LogUtil.logClass(this, "get fail 14 " + ioe.getMessage());
			ioe.printStackTrace();
			return (-1);
		}
		return (-1);
	...
	*/
	...
	public String getServletPath(RpcServlet p_r) {
		...
		for (Enumeration e = servletInUse.keys(); e.hasMoreElements();) {
			...
			RpcConnServlet rpcconnservlet = (RpcConnServlet) servletInUse
					.get(pathName);
			// LogUtil.log("HAHA checking " + pathName + " " + p_r + " " +
			// rpcconnservlet);
	...
	public boolean checkGetFileWithTime() {
		LogUtil.log("checkGetFileWithTime ");
		return (true);
	}
	...
	public int putFileBytes(byte[] putbuf, int ofs, int len) {
		try {
			if (putbuf != null) {
				ByteStream.writeInt(bs,len);
				ByteStream.loopWrite(bs,putbuf, ofs, len);
			} else {
				ByteStream.writeInt(bs,0);
				bs.flush();
				if (ByteStream.readByte(bs) != EOT)
	...
	public StreamMultiplexer multiplexWhenReturn() {
		if(nextMultiplexer == null) nextMultiplexer = new StreamMultiplexer(bs);
		return(nextMultiplexer);
	}
