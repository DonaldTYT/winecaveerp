...
class RpcRegistry
{
  ...
  public void preload(int n)
  {
    ...
    for(i=0;i<n;i++) 
      store(arr[i]);
    LogUtil.log("preload " + classname + " " + pool.size() + " Max " + maxPool);
  }
  public synchronized void store(RpcServlet rpcserv)
  {
	if(maxPool <= 0) {
		LogUtil.log("drop rpcservlet " + rpcserv);
		return;
	}
  ...
  public RpcServlet get()
  {
    ...
    if(isReflect) {
      try {
        ...
        if(constructor == null) {
          LogUtil.log("Cannot get constructor for " + classname);
        }
        ...
        LogUtil.log(e);
        LogUtil.log("getService Fail:" + e.getMessage());
        return null;
      }
    }
    return null;
  ...
  public synchronized RpcServlet getService(String service_name)
  {
    ...
    if(reg != null) {
      rpcservlet =reg.get();
    	...
    	LogUtil.log("RpcServlet get Service ("+service_name+") got null");
    }
    return(rpcservlet);
  }
