...
public class RpcClient implements RpcClientCallableWithCallback
{
  ...
  {
    if (conn == null) {
       conn = new RpcServerConnection(service);
  ...
  public RpcClient open() throws RpcException
  {
    ...
    try {
      ...
      if (debug) {
		UniLog.logClass(this, "open(): "+hostName+":"+portNumber);
      }
      Socket rpcSocket = null;
      //rpcSocket = new Socket(hostName, portNumber);
      rpcSocket = createSocket(hostName, portNumber);
      conn.setSocket(rpcSocket);
      return(this);
  ...
  public Value callSegmentWithException(String segname) throws Exception {
    try {
      ...
      return(conn.callSegment(segname,null,0));
    	...
    	//UniLog.log("conn.isDebug()="+conn.isDebug());
		if (conn.isDebug())
		   UniLog.log(e);
		throw(e);
    }
  ...
  public void removeCallback(RpcServlet p_callback) {
     removeRpcServlet(p_callback.getClass().getName());
  }
  public void addCallback(RpcServlet p_callback) {
	  setRpcServlet(p_callback.getClass().getName(), p_callback);
  }
  ...
  protected void setConnection(RpcServerConnection p_conn)
  {
  		conn = p_conn;
  }
  public RpcClient()
