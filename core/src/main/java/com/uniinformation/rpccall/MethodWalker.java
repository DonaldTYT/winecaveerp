...
public class MethodWalker
{
	...
	int getNodeType(String className)
	{
		if(className.equals("java.lang.String")) {
			...
			UniLog.log("getNodeTpye() Boolean return String type");
			return(METHOD_STRING_TYPE);
			// return(METHOD_BOOLEAN_TYPE);
		}
		UniLog.log("getNodeType unknown "  + className);
		...
		throws ClassNotFoundException
	{
		...
		if(dirty == 0 && parameterType.length >0) {
			UniLog.log("Warning !!!: Ambigous method " + method + " rejected");
			return;
		}
	...
	Object [] transformParameters(Method m,Object parameters[])
	{
		...
		// UniLog.log("start transfromParameters getParameterTypes()");
		Class ptype[] = m.getParameterTypes();
		// UniLog.log("end transfromParameters getParameterTypes()");
	...
	***/
		...
		throws Exception
	{
		...
		UniLog.log("start getMethods");
		Method []methods = o.getClass().getMethods();
		UniLog.log("end getMethods");
