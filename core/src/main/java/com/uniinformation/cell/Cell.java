...
//							if(jxValueMode != VMODE_OVERRIDED) {
	...
	public void clearEvaluatingChild(){ //use it carefully, it may generate trigger recursively!!
		UniLog.log("clearEvaulatingChildFlag called");
		if (cd != null)
...
//						UniLog.log(ex);
						UniLog.log("Recursive Evaluation Inhibited CellLabel: " + getCellLabel());
						throw ex;
					}
					cd.evaluatingChild = true;
					//UniLog.log("Evaluating columnCell 1: " + getCellLabel());
			...
			if(cd.triggerList != null) {
				...
				while(e.hasMoreElements()) {
					...
					*/
					...
					//UniLog.log("Evaluating columnCell 2 : " + getCellLabel());
					c = (Cell) e.nextElement();
					if(c == this) {
						UniLog.log("Warning !!! ignore trigger to parent cell itself");
						continue;
					}
					cd.evaluatingChild = true;
					try {
		...
		case VTYPE_INT:
			try {
				...
				if(!StringUtils.isBlank(strval)) {
					UniLog.log1("Set Non-Integer [%s] to Cell.Integer is not allowed, replaced to 0", strval);
				}
//				set(Double.NaN);
				set(0);
			}
		...
		case VTYPE_DOUBLE:
			try {
				...
				//UniLog.log("Set Non-Number to Cell.Double is not allowed, replaced to 0");
				if (!StringUtils.equalsAny(strval, "-", "")) {   //too much error msg for vincero
					UniLog.log1("Set Non-Number %s to Cell.Double is not allowed, replaced to 0", strval);
				}
//				set(Double.NaN);
				set(0.0);
			}
		...
		case VTYPE_DATETIME: {
			...
			break;
		default:
			UniLog.log(new Exception("set String to unsupported Cell type ignored type = " + jxValueType));
			return;
		}
...
//			if(jxValueMode == VMODE_OVERRIDED) {
			if(cd.isOverrided) {
				if (p_orgValue instanceof Boolean){
					...
					UniLog.log("format not supported: " + p_orgValue);
				}
			}
			else{
				if (p_orgValue instanceof Boolean){
			...
			UniLog.log("sth wrong during reverse");
			ex.printStackTrace();
		}
	}
	/***
...
//		}
		...
		case VTYPE_INT :		
			...
			try {
				...
				UniLog.log("CellException Catched restore 1 "+oldValue);
				updateReverse(oldValue);
				return(new ReturnMsg(false,ex));
			}
			break;
		case VTYPE_DOUBLE:
			...
			try {
				...
				UniLog.log("CellException Catched restore 2 " + oldValue);
				updateReverse(oldValue);
				return(new ReturnMsg(false,ex));
			}
			break;
		case VTYPE_STRING :
			...
			try {
				...
				UniLog.log("CellException Catched restore 3 " + oldValue);
				updateReverse(oldValue);
				return(new ReturnMsg(false,ex));
			}
			break;
		...
		case VTYPE_DATETIME   :
			...
			try{
				...
				UniLog.log("CellException Catched restore 4 " + oldValue);
				updateReverse(oldValue);
				return(new ReturnMsg(false,ex));
			}
			break;
...
//			UniLog.log("syncMode str override " + getStrVal());
...
//			if(cd == null) {setStrVal(""+intval);return;} else cd.setStr = ""+intval;
	...
	{
		if(intval != getIntVal()) {
//			UniLog.log("syncMode int override " + getIntVal());
			setIntVal(intval);
...
//			if(jxValueMode != VMODE_OVERRIDED) {
	...
	{
		if((d != null && d != getDateVal()) || (d == null && getDateVal() != null  && getDateVal().after(com.kyoko.common.DateUtil.minDate))) {
//			UniLog.log("syncMode date override " + getDateVal() + " <-> " + d);
			setDateVal(d);
			if(cd != null && cd.valueMapperList != null) {
				for(int i=0;i<cd.valueMapperList.size();i++) {
					cd.valueMapperList.get(i).cellMap_valchange(this);
			...
			if(jxValueMode == VMODE_PROTECTED) {
				...
				try {
					...
					UniLog.log("cell sync date newoverride exception captured");
					setMode(VMODE_PROTECTED);
					throw(ex);
				}
			} else {
...
//			if(jxValueMode != VMODE_OVERRIDED) {
	...
	{
		if(dbval != getDbVal()) {
//			UniLog.log("syncMode double override " + getDbVal());
			setDbVal(dbval);
			if(cd.valueMapperList != null) {
				for(int i=0;i<cd.valueMapperList.size();i++) {
					cd.valueMapperList.get(i).cellMap_valchange(this);
...
//		if(jxValueMode != VMODE_OVERRIDED) {
	...
	{
		if(boolval != getBoolVal()) {
//			UniLog.log("syncMode boolean override " + getBoolVal());
			setBoolVal(boolval);
			if(cd.valueMapperList != null) {
				for(int i=0;i<cd.valueMapperList.size();i++) {
					cd.valueMapperList.get(i).cellMap_valchange(this);
...
//				if (getDateVal() != null) return(getDateVal().toLocaleString()); else return("");
	...
	{
		switch(jxValueType) {
		case VTYPE_STRING : return(Integer.parseInt(StringUtil.toNumberOnly(getStrVal())));
	...
	{
		switch(jxValueType) {
		case VTYPE_STRING : return(Double.parseDouble(StringUtil.toNumberOnly(getStrVal())));
		case VTYPE_DOUBLE : return(getDbVal());
   ...
   public Hashtable getAttributes() {
	...
	public void resetValue() throws CellException
	{
		...
		*/
		...
		case VTYPE_DATETIME   :		
		case VTYPE_DATE   :		
								setDateVal(com.kyoko.common.DateUtil.zeroDate);
								if(cd != null) {
...
//		cd.cellLabel = p_label;
	...
	public void clearAllOverrides() throws CellException {
		...
		if(cd.evaluatingChild && cd.isOverrided) {
			UniLog.log("clearAllOverrider Recursive Detected, return immediately");
			return;
		}
		...
		if(cd.triggerList != null) {
			...
			for(Cell cc : cd.triggerList.values()) {
				if(cc == this) {
					UniLog.log("Warning !!! ignore clearAllOverride to parent cell itself");
					continue;
				}
