 ...
 */
public abstract class BaseDecumulator {
//	public static Date accMaxDate = DateUtil.getDate("2037/12/31");
	public Comparable MinValue = null;
...
//	public abstract void loadFromCache(Date p_date) CalculationErrorthrows Exception;
	...
	public BaseDecumulator(Comparable p_minValue)
	{
		MinValue = p_minValue;
		reset();
	}
...
//			loadFromCache(p_date);
			if(dataSet.size() == 0) {
				...
				if(dv == null) dv = newDatedValue(MinValue);
				dataSet.add(dv);
			}
			double vP1,vN1;
...
//					CoreLog.log("HAHA " + i + " " + dv.valueDate + " " + dv.pAmount + " " + dv.nAmount);
					vP1 -= dv.pAmount;
					vN1 -= dv.nAmount;
					dv.pAmount = vP1;
					dv.nAmount = vN1;
	...
	public void updateBalance(Comparable p_date,double p_deltaP,double p_deltaN,int beginOrEnd,boolean skipZeroDelta) throws CalculationErrorException
	{
		...
		if(p_deltaP == 0.0 && p_deltaN == 0.0) {
			CoreLog.log("add entry for zero delta");
		}
...
//			if(idx < 0) return(null);
	...
	{
		// p_date must less then  MinValue becauase there are no next record after dataSet at MaxValue, therefore cannot calculate the closing balance
		if(dataSet.size() > 0 && (p_date.compareTo(dataSet.get(0).valueDate) > 0) && !(p_date.compareTo(purgedDate) > 0)) {
			return(dataSet.get(0).pAmount + dataSet.get(0).nAmount);
		}
		int idx = bSearch(p_date);
	...
	*/
	...
	public void dump() {
		for(DatedValue dv : dataSet) {
			CoreLog.log("DataValue " + dv.valueDate + " " + dv.pAmount + " " + dv.nAmount);
		}
	}
}
