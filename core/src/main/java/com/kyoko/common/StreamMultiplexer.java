...
//            		CoreLog.log("flushMpxStream write " + cc + " bytes");
        ...
        */
        ...
        {
            CoreLog.log("RpcMultiplexer Created " );
            rpcs = p_rpcs;
...
// NEED HANDLING            receiver =
        ...
        public void run()
        {
        	CoreLog.log(Thread.currentThread().getName()+" Started");
            try
            {
...
//                              pendingStreams[port] = msl;
                            ...
                            CoreLog.log("msl count = " + msl.size());
                            for(int i=0;i<msl.size();i++)
                            {
                                CoreLog.log("msl" + i + " " + msl.get(i).peerid + " , " + msl.get(i).port);
                        ...
                        if (ms == null)
                        {
                            ...
                            CoreLog.log("add unaccepted tcp stream peerid " + peerid + " port " + port);
                            ms = new MpxStream(0, peerid, port);
                            msl.add(ms);
                        } else {
                            synchronized(ms) {
                            	ms.peerid = peerid;
                            	CoreLog.log("wakeup accepted tcp stream peerid " + peerid + " port " + port + " myid " + ms.id);
                            	ms.notifyAll();
                            }
                        }
                        break;
                    case ByteStream.ETB: // end stream by caller
                    	...
                    	CoreLog.log("Mpx Stream closed by remote id " + closeid);
                        ms = null;
                        synchronized(activeStreams) {
...
//                        CoreLog.log("240519 tunnel read flushed");
            ...
            {
                CoreLog.log("Exception trapped " + e.toString());
...
//                Thread.sleep(30000);
                ...
                CoreLog.log("Connect Delay cancelled "+ex);
            }
            synchronized(ms)
            {
                if(ms.peerid > 0)
                {
                        CoreLog.log("connect return id = "+ms.id);
                        return(ms);
                } else {
                	synchronized(activeStreams)
                	{
...
//                    Thread.sleep(30000);
                ...
                {
                    CoreLog.log("Accept Delay cancelled " + ex);
                    synchronized(ms)
                    {
                        if (ms.peerid <= 0)
                        {
