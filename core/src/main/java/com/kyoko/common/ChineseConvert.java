...
//						byte[] tmpch = s.getBytes("MS950");
						...
						if(tmpch .length == 1 && tmpch[0] == '?') {
							CoreLog.log(new Exception("ConvertG2B failed"));
				      	idxtobig5(idx,ch);
						   bytes[i-2] = ch[0];
						   bytes[i-1] = ch[1];
						} else {
		...
		for (int i=p_idx0; i<p_idx1; i++){
	      ...
	      int cc = idxtobig5(i,big5Bytes);
		   ...
		   int gbInt = 0;
			...
			CoreLog.log("idx:" + i + " " + str + " gbInt:" +gbInt);
			//debug
			for (int j=0; j<big5Bytes.length; j++){ 
			   CoreLog.log("big5Bytes[" + j+"]:  " + String.format("(%x)",big5Bytes[j]));
			}
			for (int j=0; j<gbBytes.length; j++){ 
			   CoreLog.log("gbBytes[" + j+"]:  " + String.format("(%x)",gbBytes[j]));
			}
