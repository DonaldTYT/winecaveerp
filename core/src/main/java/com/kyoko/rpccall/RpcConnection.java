...
//	private Socket sock;
	...
	*/
	...
	public void setDefaultServletClassName(String p_classname) {
		CoreLog.logClass(this, p_classname);
	...
	private void chkJVMVersion() {
		...
		// if (debug) CoreLog.logClass(this, vmvendor);
		if (vmvendor.startsWith("Microsoft Corp.")
			 ...
			 * String("ISO-8859-1");
			 */
			// CoreLog.logClass(this, "set encoding to Big5");
			/*
	...
	public RpcConnection(RpcServletProvider r) {
		...
		// CoreLog.logClass(this,"nomethodwalker " + nowalker);
		if (nowalker == false) {
	...
	*/
	...
	public void setTokenSupport(boolean sw) {
		...
		CoreLog.log("set tokenSupport to " + sw);
	}
	...
	public void setSocket(Socket s) {
		...
		try {
			...
			if (debug) {
				CoreLog.logClass( this, "Rpc Connection Turn On Tcp_No_Delay " + sock.getSendBufferSize());
			}
	...
	*/
	...
	private RpcServlet searchFeature(String servicename) {
		...
		if (servicename.equals(""))
			return (null);
		// CoreLog.logClass(this, "search Feature " + servicename + " " +
		...
		if (rpcservlet != null) {
			...
			CoreLog.logClass(this, "Provider return null servlet");
		}
		return (rpcservlet);
	}
	...
	final public void run() {
		if (debug)
			CoreLog.logClass(this, "Connection runs " + Thread.currentThread());
		try {
			/*
			...
			for (;;) {
				...
				if (cc != SOH) {
					CoreLog.logClass(this, "Protocol error cannot get SOH");
					// changed by lai Fri Aug 10 23:30:31 HKG 2001 return;
					break;
				}
				CoreLog.log("240519 got SOH");
				realRpcServer();
				if (exitFlag)
					break;
				if(nextMultiplexer != null) {
					CoreLog.log("Converting rpc server to Mpx");
					nextMultiplexer.start();
					CoreLog.log("mpx started accepting channel 1 as rpc server");
					ByteStream mbs = nextMultiplexer.accept(1);
					CoreLog.log("channel 1 returned, setting rpcserver byte stream to mpx channel1");
					bs = mbs;
					nextMultiplexer = null;
				}
			}
		} catch (java.io.InterruptedIOException ioe) {
			CoreLog.logClass(this, this + " timeout");
		} catch (Exception e) {
		}
		...
		if (rpcclient != null)
			...
			*/
		if (debug)
			CoreLog.logClass(this, "Connection Gone" + Thread.currentThread());
		return;
	}
	...
	private int matchdirlist(String fname) {
		CoreLog.log("matchdirlist skipped.");
		return (1);
		/*
	...
	public int putFile(InputStream input, int maxlen, String filename,
		...
		try {
			...
			case NAK:
				...
				if (v == null)
					return (-1);
				CoreLog.logClass(this, "get fail 1 " + v.toString());
				v = null;
				return (-1);
			}
		} catch (IOException ioe) {
			CoreLog.logClass(this, "get fail 2 " + ioe.getMessage());
			return (-1);
		}
		return (-1);
	}
	...
	public int putFile(DataInput dataInput, int maxlen, String filename,
		...
		try {
			...
			case NAK:
				...
				if (v == null)
					return (-1);
				CoreLog.logClass(this, "get fail 3 " + v.toString());
				v = null;
				return (-1);
			}
		} catch (IOException ioe) {
	...
	private int rpcserver_putfile(DataInput input, int maxlen, int maxrate) {
		...
		try {
			...
			CoreLog.logClass(this,
					"rpcserver_putfile I/O error " + e.getMessage());
			CoreLog.log(e);
			return (-8);
		} catch (RpcException e) {
			CoreLog.logClass(this, "rpcserver_putfile error " + e.getMessage());
			CoreLog.log(e);
			return (-10);
		}
		return (writecnt);
	...
	public int getFile(DataOutput dataOutput, int maxlen, String filename,
		...
		try {
			...
			case NAK:
				...
				if (v == null)
					return (-1);
				CoreLog.logClass(this, "get fail 4 " + v.toString());
				v = null;
				return (-1);
			}
		} catch (IOException ioe) {
			CoreLog.logClass(this, "get fail 5 " + ioe.getMessage());
			return (-1);
		}
		return (-1);
	}
	...
	public int getFile(OutputStream out, int maxlen, String filename,
		...
		try {
			...
			case NAK:
				...
				if (v == null)
					return (-1);
				CoreLog.logClass(this, "get fail 6 " + v.toString());
				v = null;
				return (-1);
			}
		} catch (IOException ioe) {
			CoreLog.logClass(this, "get fail 7 " + ioe.getMessage());
			return (-1);
		}
		return (-1);
	}
	...
	private int rpcserver_getfile(DataOutput outputstream) {
		try {
			...
			CoreLog.logClass(this, "rpcserver_getfile " + e.getMessage());
			CoreLog.log(e);
			return (-1);
		}
		return (0);
						...
						for (varArgStart = 0; varArgStart < j; varArgStart++) {
							if (ptype[varArgStart] != margclasses[varArgStart]) {
								...
								if (pname2.equals("java.lang.Integer")
									...
									CoreLog.logClass(this, "argument "
											+ varArgStart + " " + pname2);
									CoreLog.logClass(this,
	...
	private Object invokeServlet(RpcServlet serv, String methodname,
		...
		if (nowalker) {
			...
			try {
				...
				varArgStart = -1;
				CoreLog.log("checkVarArg 1");
				mymethod = checkVarArg(myclass, margclasses, methodname,
						margobjects);
				CoreLog.log("checkVarArg 2");
				if (mymethod == null) {
					CoreLog.logClass(this, "myclassname=" + myclass.getName());
					CoreLog.logClass(this, "methodname=" + methodname);
					for (int z = 0; z < margobjects.length; z++) {
						CoreLog.logClass(this, "margclasses[" + z + "]="
								+ margclasses[z]);
					}
			...
			if (walker == null) {
				...
				if (debug)
					CoreLog.log("start build " + methodFullName);
				walker = new MethodWalker(methods, methodname);
				if (debug)
					CoreLog.log("end build " + methodFullName);
				walkerHash.put(methodFullName, walker);
			}
			// if(debug) CoreLog.log("start search " + methodFullName);
			mymethod = walker.searchMethodTree(margobjects);
			// if(debug) CoreLog.log("end search " + methodFullName);
			if (mymethod == null) {
				CoreLog.logClass(this, "method=" + methodFullName);
				for (int z = 0; z < margobjects.length; z++) {
					CoreLog.logClass(this, "margclasses[" + z + "]="
							+ margclasses[z]);
				}
	...
	private int realRpcServer() {
		...
		try {
			...
			if (rpccallid == RPCGETFILEID || rpccallid == RPCGETFILEID2
				...
				try {
					...
					if (rpccallid == RPCGETFILEIDWITHTIME) {
						CoreLog.log("sendFileTime true, sending DC2 and filetime");
			...
			Value v = null;
			// CoreLog.logClass(this, "callname " + callname);
			...
			if (tokencnt == 1) {
				...
				// CoreLog.logClass(this, "Method " + methodname);
...
//				return (-1);
				...
				arglist = null;
				if (rpccallid != RPCCALLSEGMENTID_NORETURN) {
					CoreLog.log("rpc " + callname + " not found");
			...
			try {
				...
				for (i = 0; i < numarg; i++) {
					...
					case ISDVALUE:
						...
						// value properly.
						// CoreLog.log("HAHA in rpcserverconnection rpcservercallback "
						// + dval);
						if (dval.toInt() <= 0) {
							margobjects[i] = null;
						} else {
							...
							margobjects[i] = tdate;
							// CoreLog.log("HAHA in rpcserverconnection rpcservercallback jd = "
							// + tdate);
						}
						break;
					}
				...
				if (nowalker) {
					...
					try {
						...
						CoreLog.log("checkVarArg 2");
						if (mymethod == null) {
							CoreLog.logClass(this, "myclassname=" + myclassname);
							CoreLog.logClass(this, "methodname=" + methodname);
							for (int z = 0; z < margobjects.length; z++) {
								CoreLog.logClass(this, "margclasses[" + z + "]="
										+ margclasses[z]);
					...
					if (debug)
						CoreLog.log("start search " + methodFullName);
					mymethod = walker.searchMethodTree(margobjects);
					if (debug)
						CoreLog.log("end search " + methodFullName);
					if (mymethod == null) {
						if (debug) {
							CoreLog.logClass(this, "method=" + methodFullName);
							for (int z = 0; z < margobjects.length; z++) {
					...
					}
					{
						 ...
						 * (IOException ex) { CoreLog.logClass(this,
						 * "exteption trapped inside invoke"); CoreLog.log(ex);
						 * return(-1); }
						 */
					}
				}
				...
				if (debug)
					CoreLog.logClass(this, "return type " + rettype);
				if (retClass
			...
			CoreLog.logClass(this, "Error in rpc protocol " + e.getMessage());
			CoreLog.log(e);
			cc = -1;
		}
		return (cc);
	...
	public void stopService() {
		...
		try {
			...
			CoreLog.logClass(this, "stop service fail");
		}
		provider = null;
		servletInUse = null;
		bs = null;
			...
			if (strval.getEncoding() != null) {
				byte[] buf2;
				{
					...
					for (int i = 0; i < cr.length; i++) {
						int ci;
						ci = cr[i];
						CoreLog.log("UTF8 convert C " + i + " " + ci);
					}
				}
				/*
				 * if(simToTran) { buf2 =
				...
				if (strval.getEncoding().equals("SJIS"))
					...
					buf[1] = 'u';
				CoreLog.log("HAHA_UTF8 A " + buf2.length);
				for (int i = 0; i < buf2.length; i++) {
					CoreLog.log("HAHA_UTF8 B " + i + " " + buf2[i]);
					buf[i + 2] = buf2[i];
				}
			} else {
				if (simToTran) {
	...
	private Value readValue() throws IOException {
		...
		// CoreLog.logClass(this, "readvalue type = " + cc);
		switch (cc) {
		case V_NULL:
		...
		case V_LONGSTR:
			...
			if (len > 0) {
				...
				if (offset >= 2 && buf[0] == -1 /* && buf[1] == 'u' */) {
					switch (buf[1]) {
					case 'j':
						CoreLog.log("SJIS convert");
						return (new Strval(new String(buf, 2, offset - 2,
								"SJIS")));
					default:
						CoreLog.log("UTF8 convert");
						/*
						 ...
						 */
						{
							for (int i = 0; i < buf.length; i++) {
								CoreLog.log("UTF8 convert A " + i + " " + buf[i]);
							}
							...
							for (int i = 0; i < cr.length; i++) {
								int ci;
								ci = cr[i];
								CoreLog.log("UTF8 convert B " + i + " " + ci);
							}
	...
	// throws RpcException, ClassNotFoundException
	{
		if (debug) {
			if (argcnt == 0)
				CoreLog.logClass(this, "RpcConnection callsegment " + returnFlag
						+ " " + segname + " " + argcnt);
		}
		try {
			int i = 0;
			...
			if (argcnt > 0) {
				...
				while (enum0.hasMoreElements()) {
					...
					*/
					...
					if (aClass.isAssignableFrom(Class
							...
							if (rtoken != null) {
								...
								if (idx == 0) {
									...
									vlist.addElement(new TokenStringVal(strret,
											tokenidx));
									// CoreLog.log("init new Token arg " +
									// tokenidx + " " + strret);
								} else {
									vlist.addElement(new TokenVal(idx));
									// CoreLog.log("use Token arg " + idx);
								}
							} else {
						...
						vlist.addElement(new Longval(intret.intValue()));
						CoreLog.logClass(this, "not support long type"); // andrew190323:
																		// server
																		// side
																		// will
																		// treat
						...
						// CoreLog.log("HAHA in RpcServerConnection date getTime "
						// + dateret.getTime());
						// CoreLog.log("HAHA in RpcServerConnection date getGmtOffset "
						// + DateUtil.getGmtOffset());
						// CoreLog.log("HAHA in RpcServerConnection date argument "
						// + dateret + " -> " + l);
				...
				if (debug) {
					CoreLog.logClass(this, sb.append("]").toString());
				}
			} else {
				vlist = new Vector();
			}
...
//			CoreLog.log("before call remote server");
			Value v =
					(callRemoteServer(segname,
					returnFlag ? (noAckFlag ? RPCCALLSEGMENTID_NOACK
							: RPCCALLSEGMENTID) : RPCCALLSEGMENTID_NORETURN,
					vlist.elements(), vlist.size()));
// 			CoreLog.log("after call remote server");
			return(v);
		} catch (Exception e) {
			CoreLog.log("call fail: " +e.getMessage());
			if (debug) {
				CoreLog.log(e);
			}
			throw (e);
	...
	private Value callRemoteServer(String segname, int callid,
		...
		try {
				...
				if (rtoken != null) {
					...
					if (idx == 0) {
						...
						*/
						...
						// CoreLog.log("init new Token seg " + tokenidx + " " +
						// segname);
					} else {
						ByteStream.writeShort(bs,0);
						ByteStream.writeInt(bs,idx);
						ByteStream.writeByte(bs,STX);
						// CoreLog.log("use Token seg " + idx);
					}
				} else {
					byte[] ba = segname.getBytes();
					ByteStream.writeShort(bs,ba.length);
			...
			if (debug)
				CoreLog.logClass(this, "Couldn't get I/O for the connection");
			if (debug)
				CoreLog.log(e);
			throw (e);
		} catch (RpcException e) {
	...
	public void setDebug(boolean p_flag) {
		debug = p_flag;
		if (debug) {
			CoreLog.log("Server Connection setDebug " + p_flag);
		}
	}
	...
	*/
	...
	public int putFileWithCallback(InputStream input, int maxlen,
		...
		try {
			...
			case NAK:
				...
				if (v == null)
					return (-1);
				CoreLog.logClass(this, "get fail 8 " + v.toString());
				v = null;
				return (-1);
			}
		} catch (IOException ioe) {
			CoreLog.logClass(this, "get fail 9 " + ioe.getMessage());
			return (-1);
		}
		return (-1);
	}
	...
	public int putFileWithCallback(DataInput dataInput, int maxlen,
		...
		try {
			...
			case NAK:
				...
				if (v == null)
					return (-1);
				CoreLog.logClass(this, "get fail 10 " + v.toString());
				v = null;
				return (-1);
			}
		} catch (IOException ioe) {
	...
	private void fileProgress(String p_methodname, Vector arglist) {
		...
		try {
			...
			if (serv == null) {
				CoreLog.log("fileProgress fail 1 " + p_methodname + " "
						+ myclassname);
				return;
			}
			...
			CoreLog.log("fileProgress fail 2 " + p_methodname + " "
					+ myclassname);
			CoreLog.log(e.getMessage());
		}
	}
	...
	public int getFileWithCallback(DataOutput dataOutput, int maxlen,
		...
		try {
			...
			case NAK:
				...
				if (v == null)
					return (-1);
				CoreLog.logClass(this, "get fail 11 " + v.toString());
				v = null;
				return (-1);
			}
		} catch (IOException ioe) {
			CoreLog.logClass(this, "get fail 12 " + ioe.getMessage());
			return (-1);
		}
		return (-1);
	}
	...
	public int getFileWithCallback(OutputStream out, int maxlen,
		...
		try {
			...
			case NAK:
				...
				if (v == null)
					return (-1);
				CoreLog.logClass(this, "get fail 13 " + v.toString());
				v = null;
				return (-1);
			}
		} catch (IOException ioe) {
			CoreLog.logClass(this, "get fail 14 " + ioe.getMessage());
			ioe.printStackTrace();
			return (-1);
		}
		return (-1);
	...
	*/
	...
	public String getServletPath(RpcServlet p_r) {
		...
		for (Enumeration e = servletInUse.keys(); e.hasMoreElements();) {
			...
			RpcConnServlet rpcconnservlet = (RpcConnServlet) servletInUse
					.get(pathName);
			// CoreLog.log("HAHA checking " + pathName + " " + p_r + " " +
			// rpcconnservlet);
	...
	public boolean checkGetFileWithTime() {
		CoreLog.log("checkGetFileWithTime ");
		return (true);
	}
