...
public class ZkBiComposerAggregate extends ZkBiComposerBase{
...
//		if(listboxHeightAdjust != 0) 
    ...
    public void buildBrowserWindow(final BiResult result,final Component comp, int p_sortIdx, boolean p_sortDesc){
	    ...
	    final ZkForm zkf1 = zkfName == null ? null :  new ZkForm(rpth,zkfName);
		try {
			rptCol = new CellCollection();
			key = "AGGREPORT_"+result.getView()+"_"+getPageid()+"_"+getSessionHelper().getVcode();
//			String key = "AGGREPORT_"+result.getView()+"_"+getSessionHelper().getVcode();
			JSONObject jo = FilingUtil.getJson(getSessionHelper().getAgent(), null , key);
			if(jo == null) {
				String oldkey = "AGGREPORT_"+result.getView()+"_"+getSessionHelper().getVcode();
				jo = FilingUtil.getJson(getSessionHelper().getAgent(), null , oldkey);
			}
	...
	protected void saveAggregateConfig() throws Exception {
		JSONObject jo = CellCollectionToJsonInterface.CellCollectionToJSON(rptCol);
