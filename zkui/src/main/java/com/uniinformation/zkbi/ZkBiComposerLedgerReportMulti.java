...
public class ZkBiComposerLedgerReportMulti extends ZkBiComposerReport {
...
//		cumulatorComparable = ((BiLedgerReportInterface) br).getCumulatorComparable();
		return(br);
   	}
	@Override
...
//		lpi.setGroupBy(rptCol.getCellInt("rgGroupBy"));
		...
		*/
		try {
			createBirtDoc(((BiLedgerReportInterface) result).getFilterStr(), minDate, maxDate, dsList, designRes, outputFileName);
		} catch (Exception e) {
			e.printStackTrace();
...
//						hs.add("TST01");
	...
	protected ReturnMsg setAdditionalQueryCondition(BiResult result) {
		...
		if(!d0.after(DateUtil.minDate)) {
			result.addCustomCondition(((BiLedgerReportInterface) result).getCumulatorColumn() + " <= '" + DateUtil.toDateString(d1, "yyyy/mm/dd")+"'");
		} else {
			if(!d1.after(DateUtil.minDate)) {
				result.addCustomCondition(((BiLedgerReportInterface) result).getCumulatorColumn() + " >= '" + DateUtil.toDateString(d0, "yyyy/mm/dd") + "'");
			} else {
				result.addCustomCondition(((BiLedgerReportInterface) result).getCumulatorColumn() + " between '" + DateUtil.toDateString(d0, "yyyy/mm/dd") + "' and '" + DateUtil.toDateString(d1, "yyyy/mm/dd") + "'");
			}
		}
		/*
