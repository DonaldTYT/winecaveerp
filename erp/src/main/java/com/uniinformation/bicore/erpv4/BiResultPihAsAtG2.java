...
public class BiResultPihAsAtG2 extends BiResultAgingAsAt{

	public BiResultPihAsAtG2(BiResult p_parent, BiView p_view, SelectUtil p_su, Vector p_tabList, String p_whereStr,
			SessionHelper p_sh, boolean p_allowLookupItemList) throws CellException {
		super(p_parent, p_view, p_su, p_tabList, p_whereStr, p_sh, p_allowLookupItemList);
		setAsAtColumn("sih_date");
//		setSkipSummaryColumns(new HashSet<String>(Arrays.asList("sih_total","sih_osbal")));
	}
	...
	protected HashSet<BiTable> addExtraWhereStr(Wherecl p_where,HashSet<BiTable> p_hash)
	{
		...
		p_where.andUniop("crh_date", "<=", asAtDate);
	...
	public boolean skipForPivot(String p_aggLabel,AggregateOrPivotHeader p_aop) {
		boolean skip = super.skipForPivot(p_aggLabel,p_aop);
		if(skip) return(skip);
		if(p_aggLabel.equals("sih_total") ||
		   p_aggLabel.equals("sih_osbal")
				) {
			if(!p_aop.groupColumnContains("sih_cid")) return(true);
		}
		return(false);
	}
	
	@Override
	public boolean skipForSummary(String p_aggLabel,AggregateOrPivotHeader p_aop) {
		boolean skip = super.skipForSummary(p_aggLabel,p_aop);
		if(skip) return(skip);
		if(p_aggLabel.equals("sih_total") ||
		   p_aggLabel.equals("sih_osbal")
				) {
			if(!p_aop.rowColumnContains("sih_cid")) return(true);
		}
		return(false);
	}
