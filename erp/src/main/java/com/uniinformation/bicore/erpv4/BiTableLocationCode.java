...
public class BiTableLocationCode extends BiTable{

	public BiTableLocationCode(CellCollection parent, String p_tableName, String p_dbtName, String p_eName,
			String p_cName, String p_selectWhere, String p_primaryKey, String p_serialid) {
	...
	protected Wherecl getFieldUniqueListAppendWhere(BiField p_fd,SessionHelper p_sh) {
		...
		if(Erpv4Config.isMultiCompany(p_sh)) {
			if(!p_sh.hasAccessRight("#multicomp")) {
				...
				wcl.appendString(" and locationcode.loc_cocode = '"+Erpv4Config.getDefaultCoCode(p_sh)+"'").stripAnd();
			}
		}
		return(wcl);
	}
