...
public class ExcelCellCollection extends BiCellCollection {
	...
	public class BicoreDataSet {
		...
		public BicoreDataSet(String p_brName,int p_startRow,int p_endRow,String[] p_columns) throws Exception {
			startRow = p_startRow;
			endRow = p_endRow;
			if(startRow < 0 ) startRow = getIdx();
			if(endRow < 0 ) endRow = getIdx();
			if(StringUtils.isBlank(p_brName)) ssbr = br; else ssbr = ExcelWorkSheetCache.getBrFromCache(br.getSessionHelper(),p_brName);
...
//			Variable vEnd = ((Expression) toCell).getVariable();
			...
			if(rowIndexPair.getLeft()) {
				...
				startRow = rowIndexPair.getRight() + getIdx();
			}
			rowIndexPair = vEnd.getRowIndex();
			if(rowIndexPair.getLeft()) {
				endRow = rowIndexPair.getRight();
			} else {
				endRow = rowIndexPair.getRight() + getIdx();
			}
			...
			if(varName.getLeft() == null) {
				...
				ssbr = ExcelWorkSheetCache.getBrFromCache(br.getSessionHelper(),varName.getLeft());
			}
			if(ssbr != null && ssbr.getSessionHelper().getBiSchema().getSchemaAutotranslate()) {
				lhLANG = br.getSessionHelper().getLHLang();
			}
			/*
...
//				int endColPosition = sbr.getColumnObjectPositionHash().get(endName);
	...
	*/
	 ...
	 *  abc[0] -> first row
	 *  abc[i] -> ith row
	 *  abc[-x] -> currrent index - x row
	 *  abc[+x] -> currrent index + x row
	 * Range Variable :
	 *  [abc[0]:abc[]] -> whole column of abc
	 *  [abc[*] -> whole column of abc
	 *  [abc[0]:abc -> first row to current row of abc
	 *  [abc : abc[] -> current row to last row of abc
	 */
	private enum FuncName { 
		...
		,FUNC_jsonGet
		,NOT_DEFINED }
	public ExcelCellCollection(BiCellCollection p_col, BiResult p_br) {
		super(p_col, p_br);
...
//				UniLog.log1("HAHA240215 ignore DATEDIF return 0");
			...
			case FUNC_excelDAY:  {
				...
				Cell c = ((Expression) args.get(0)).eval(null);
				return(super.evalFunction("dayofdate", 
							new VectorUtil().addElement(c.getObject()).toVector()
						));
			}
...
//								Object o = ds.sbr.getResultTrObject(false,ds.colPosition[0],i);
			...
			case FUNC_defaultLdesc: {
				int lcrg = BiConfig.getDefaultLcrg(br.getSessionHelper());
				if(lcrg > 0) {
					return(BiConfig.getLcDesc(br.getSessionHelper(),lcrg));
				} else {
					return("");
				}
			}
			...
			case FUNC_defaultLcrg: {
				return(BiConfig.getDefaultLcrg(br.getSessionHelper()));
			}
			...
			*/
			...
			String s = BiSchema.getUniqueFieldEx(br,tabName,fieldName,format,rg,reuse);
			return(s);
		}
...
//				sheetName = com.uniinformation.bicore.DataModelHelper.workSheetNameToViewName(br.getSessionHelper(), p_varname.substring(0,sheetNameIdx));
				viewName = br.getSessionHelper().getDbName()+"."+p_varname.substring(0,sheetNameIdx);
	...
	@Override
	public Object evalVariable(String p_varname, int p_idx) {
		if(getIdx() < 0) {
			if(p_idx == getIdx()) {
				Pair<String,String> varName = null;
				try {
					varName = getPairedName(p_varname);
					if(varName.getLeft() == null) {
						return(super.evalVariable(p_varname));
					}
				} catch (Exception ex) {
					UniLog.log(ex);
				}
		...
		BiResult varBr=null;
		if(varName.getLeft() == null) varBr = br; else {
			try {
				varBr = ExcelWorkSheetCache.getBrFromCache(br.getSessionHelper(),varName.getLeft());
			} catch (Exception ex) {
				UniLog.log(ex);
			}
			if(varBr == null) return(0.0);
		}
		if(varName.getLeft() == null && getIdx() >= 0 && p_idx == getIdx()) {
			return(getCell(varName.getRight()).getObject());
			/*
			if(varBr.getCurrentCollection() == null) {
				return(getCell(varName.getRight()).getObject());
			} else return(varBr.getCell(varName.getRight()).getObject());
			*/
		} else {
			int ridx = p_idx;
			if( ridx >= varBr.getRowCount()) {
				ridx = varBr.getRowCount()-1;
			}
			/*
			int idx = varBr.getColumnObjectPositionHash().get(varName.getRight());
			if(idx >= 0) { 
				try {
					Object o = varBr.getResultTrObject(false,idx,p_idx);
					if(o != null) return(o);
				} catch (Exception ex) {
					UniLog.log(ex);
				}
			...
			*/
			try {
				Object o = varBr.getColumnValueFromCache(varName.getRight(), ridx);
				if(o != null) return(o);
			} catch (Exception ex) {
				UniLog.log(ex);
			}
			Cell cc = varBr.getCell(varName.getRight());
			switch(cc.getType()) {
			case Cell.VTYPE_DATE  : return(DateUtil.zeroDate);
			case Cell.VTYPE_INT   : return(0);
			case Cell.VTYPE_DOUBLE: return(Double.NaN);
	...
	public Object evalVariableRelative(String p_varname, int p_idx) throws Exception {
		return(evalVariable(p_varname, p_idx + getIdx()));
	}
	@Override
	public Object evalVariable(String p_varname) {
		return(evalVariable(p_varname, getIdx()));
	}
}
