...
//   		}
	...
	public AfsPrtdocPrintInvoice(BiResultErpv4 p_br) throws Exception {
		super(p_br, getDefaultPrtdocJson(p_br), null);
	}
	
	private static PrtdocJson getDefaultPrtdocJson(BiResultErpv4 p_br) throws Exception {
		return PrtdocJson.newPrtdocJson(
				StringUtils.defaultString(p_br.getCellString("invh_cocode"), Erpv4Config.getDefaultCoCode(p_br.getSessionHelper())), 
				StringUtils.defaultString(Erpv4Config.getString(p_br.getSessionHelper(), "InvPaperType"), "A4P"), 
				StringUtils.defaultString(Erpv4Config.getString(p_br.getSessionHelper(), "InvDocCode"), "GENINV01"), 
				"erpv4_printDocument");
	}
    ...
    protected void printGeneralAndQuotationInvoice() throws Exception
    {
        ...
        //this.ppj.addHeaderImage("logo", "logo/afslogo4.png", 0, 0, 0, 0);
        ...
        sb.append("\nTel : " + this.col.getCellString("invh_tel"));
        if (!col.getCellString("invh_fax").equals(""))
        	sb.append("          Fax : " + this.col.getCellString("invh_fax"));
        this.ppj.addHeaderField("cvname", sb.toString(), 0, 20, 20, 320);
        ...
        this.ppj.addHeaderField("dflabel", "Date :", 0, 20);
        this.ppj.addHeaderField("dfvalue", DateUtil.toDateString(this.col.getCell("invh_date").getDate(), "dd mmm, yyyy"), 0, 20);
        if(!col.getCellString("invh_quonum").equals("")) {
        this.ppj.addHeaderField("dflabel", "Ref. no. :", 0, 40);
        this.ppj.addHeaderField("dfvalue", this.col.getCellString("invh_quonum"), 0, 40);
        }
        ...
        */
        if (StringUtils.isNotBlank(this.col.getCell("invh_projecttitle").getString())) {
        ...
        */
        ...
        if(this.col.testCell("invh_quodeli") != null) {
        if (!this.col.getCellString("invh_quodeli").equals("")) {
            remark = String.valueOf(remark) + "\r" + this.col.getCellString("invh_quodeli");
        }
        }
        ...
        if (tr.getRecordCount() > 0) {
            ...
            if (this.col.getCellInt("invh_bankinfo") > 0)
            	bankInfo = tr.getFieldString("co_payment2");
            else
            	bankInfo = tr.getFieldString("co_payment");
        }
        this.ppj.addBottomField("val_remark1", "Remarks :", 0, 0, 20, 0);
        this.ppj.addBottomField("val_remark", remark, 0, 20, 20, 0);
        if (bankInfo != null) {
        	int count = 0;
        	if (remark != null)
				count = Math.max(ChnftrParser.splitText(remark, "ariblk", "chinese", 9, 400).size() - 5, 0);
        	this.ppj.addBottomField("val_remark1", "Bank Information :", 0, 125 + 20 * count, 20, 0);
			this.ppj.addBottomField("val_remark", bankInfo, 0, 145 + 20 * count, 20, 0);
        }
    }
    ...
    protected void printDoInvoice() throws Exception {
    		...
    		if(Erpv4Config.isMultiCompany(sh)) {
    			...
    			//ppj.addHeaderImage("logo", "logo/afslogo4.png", 0, 0, 0, 0);
    		}
    		...
    		*/
    		...
    		StringBuilder sb = new StringBuilder(addr);
    		sb.append("\nTel : " + this.col.getCellString("stm_tel"));
    		ppj.addHeaderField("cvname",sb.toString(),0,20,20,320);
    		...
    		ppj.addHeaderField("dfvalue", " "+DateUtil.toDateString( col.getCell("stm_date").getDate(),"yyyy/mm/dd") ,-5,20);

    		int yofs = 40;
    		if(!col.getCellString("stm_ref1").equals("")) {
    			ppj.addHeaderField("dflabel", "Ref. no. :", 0, yofs);
    			ppj.addHeaderField("dfvalue", col.getCellString("stm_ref1"), 0, yofs);
    			yofs += 20;
    		}
    		
    		Cell dd = col.testCell("sih_duedate");
    		if(dd != null) {
    			java.util.Date d0 = col.getCell("stm_date").getDate();
    			java.util.Date d1 = dd.getDate();
    			if(d0 != null && d0.after(DateUtil.minDate)) {
    				int nd = DateUtil.dateToSqlDateInt(d1) - DateUtil.dateToSqlDateInt(d0);
    				if(nd > 0) {
    					ppj.addHeaderField("dflabel", "Term :", 0, yofs);
    					ppj.addHeaderField("dfvalue", "" + nd + " days",0,yofs);
    					yofs += 20;
    				} else {
    					ppj.addHeaderField("dflabel", "Term :", 0, yofs);
    					ppj.addHeaderField("dfvalue", "C.O.D.",0,yofs);
    					yofs += 20;
    				}
    		...
    		if(!col.getCellString("svloc_desp").equals("")) {
    			ppj.addHeaderField("cvname1","DELIVERY TO :",370,-20);
    			ppj.addHeaderField("cvname",col.getCellString("svloc_desp"),370,0,0,0);
    			addr = col.getCellString("svloc_addr1").trim() + " " +  col.getCellString("svloc_addr2").trim() + " " + col.getCellString("svloc_city").trim();
    			ppj.addHeaderField("cvname",addr,370,20,20,320);
    		}
...
//    			ppj.setUnderLine(false);
    		...
    		if(col.testCell("stm_grosstotal") != null) {
    			...
    			/*ppj.addBottomField("hdr_total","Sub-Total:",0,ofs);
    			ppj.addBottomField("val_total",col.getCell("stm_grosstotal").getColumnDisplayString(),0,ofs);
    			ofs += 20;*/
    			plist.add(Pair.of("Sub-Total :", col.getCell("stm_grosstotal").getColumnDisplayString()));
    		}
    		if(col.getCellDouble("stm_discount") != 0) {
    			...
    			ofs += 20;*/
    			plist.add(Pair.of("Less Discount :", col.getCell("stm_discount").getColumnDisplayString()));
    		}
    		...
    		if(col.getCellDouble("stm_fref1") != 0) {
    			...
    			ofs += 20;*/
    			plist.add(Pair.of(getServiceName(col.getCellString("stm_salescode1"))+" :", col.getCell("stm_fref1").getColumnDisplayString()));
    		}
    		if(col.getCellDouble("stm_fref2") != 0) {
    			...
    			ofs += 20;*/
    			plist.add(Pair.of(getServiceName(col.getCellString("stm_salescode2"))+" :", col.getCell("stm_fref2").getColumnDisplayString()));
    		}
    		...
    		ppj.addBottomField("val_total",col.getCell("stm_nettotal").getColumnDisplayString(),0,ofs);
   			ofs += 20;*/
   			plist.add(Pair.of("Net Total :", col.getCell("stm_nettotal").getColumnDisplayString()));
    		if(col.getCellDouble("stm_deposit") != 0) {
    			...
    			ofs += 20;*/
    			plist.add(Pair.of("Less Deposit :", col.getCell("stm_deposit").getColumnDisplayString()));
    			plist.add(Pair.of("Invoice Amount :", col.getCell("stm_invamount").getColumnDisplayString()));
    		}
   			plist.add(Pair.of("Balance Amount :", col.getCell("sih_osbal").getColumnDisplayString()));
   			if (plist.size() > 5)
   				ofs -= 8;
   			for (Pair<String, String> p : plist) {
    			ppj.addBottomField("hdr_total",p.getLeft(),0,ofs);
    			ppj.addBottomField("val_total",p.getRight(),0,ofs);
    			if (plist.size() > 7)
    				ofs += 13;
    			else if (plist.size() > 6)
    				ofs += 15;
			...
			if(tr.getRecordCount() > 0) {
   			...
   			ppj.addBottomField("val_remark1","Remarks :",0,0,20,0);
   			ppj.addBottomField("val_remark",remark,0,20,20,0);
   			if (bankInfo != null) {
   				int count = 0;
   				if (remark != null)
   					count = Math.max(ChnftrParser.splitText(remark, "ariblk", "chinese", 9, 400).size() - 5, 0);
   				ppj.addBottomField("val_remark1", "Bank Information :", 0, 125 + 20 * count, 20, 0);
   				ppj.addBottomField("val_remark", bankInfo, 0, 145 + 20 * count, 20, 0);
   			}
