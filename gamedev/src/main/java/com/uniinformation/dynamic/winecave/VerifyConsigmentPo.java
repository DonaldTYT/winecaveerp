...
import com.uniinformation.bicore.wc.BiResultStockOutEx;
...
public class VerifyConsigmentPo extends BiActionHandler implements JxActionListener {

	BiResultStockOutEx br;

	public VerifyConsigmentPo(ZkBiComposerBase p_bibase) {
		super(p_bibase);
	...
	public ReturnMsg beforeAction(BiResult p_result,int cnt) {
		br = (BiResultStockOutEx) p_result;
		try {
			br.beginWork();
		} catch(Exception ex) {
			UniLog.log(ex);
	...
	public ReturnMsg processAction(BiResult p_br,int p_idx) {
		...
		try {
			List<String> errList = br.verify_only();
	...
	public boolean isDisabled(BiResult p_br,boolean p_isBatch) {
		...
		if(!p_br.getSessionHelper().hasAccessRight("#prtinv")) return(true);
