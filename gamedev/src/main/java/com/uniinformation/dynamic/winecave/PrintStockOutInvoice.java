...
public class PrintStockOutInvoice  extends BiActionHandler implements JxActionListener {

	public PrintStockOutInvoice() {
		super(null);
		// TODO Auto-generated constructor stub
	}
	public PrintStockOutInvoice(ZkBiComposerBase p_bibase) {
		super(p_bibase);
		// TODO Auto-generated constructor stub
	}
...
//	void addOneStockOut(BiResult br) throws Exception {
...
//	}
	...
	String printOneStockOutInvoices(RpcClient rpc,int p_mrg) throws Exception {
...
//		}
		...
		args.add("NORMAL");
		args.add("LPTRAW");
		Value val = rpc.callSegment("winecave_print_invoice",args);
...
//		addOneStockOut(br);
		...
		rpc.callSegment("setCocodeBaseccy",
				new VectorUtil()
				.addElement( Erpv4Config.getDefaultCoCode(br.getSessionHelper()))
				.addElement( Erpv4Config.getBaseCcy(br.getSessionHelper(),Erpv4Config.getDefaultCoCode(br.getSessionHelper())))
				.toVector()
				);
...
//		invList.sort(null);
//		if(invHash.size() == 1) {
//			String docUrl = printStockOutInvoices(rpc,invList.get(0),invHash.get(invList.get(0)));
			String docUrl = printOneStockOutInvoices(rpc,br.getCellInt("stm_mrg"));
			if(docUrl != null) {
				...
				ps.setChnftrGetImageInterface(new ChnftrGetImageInterface(){
					...
					public byte[] getImage(String p_key) {
						...
						try {
							return(br.getSessionHelper().newErpFileToByteArray(url));
						} catch (Exception ex) {
							UniLog.log(ex);
							return(null);
						}
...
//				ChnftrParser ps = new ChnftrParser(is,""); // print as A4 , ok
				...
				if(sessionHelper.isMobileDevice()) {
					ZkUtil.openPdfInlineFromStream(bis, "application/pdf",sessionHelper);
				} else {
