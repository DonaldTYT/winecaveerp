...
public class VerifyStockOutChg extends BiActionHandler implements JxActionListener {

	BiResultStockOutChg br;

	public VerifyStockOutChg(ZkBiComposerBase p_bibase) {
		super(p_bibase);
	...
	public ReturnMsg beforeAction(BiResult p_result,int cnt) {
		br = (BiResultStockOutChg) p_result;
		try {
			br.beginWork();
		} catch(Exception ex) {
			UniLog.log(ex);
	...
	public boolean isDisabled(BiResult p_br,boolean p_isBatch) {
		...
		if(!p_br.getSessionHelper().hasAccessRight("#detoutchg")) return(true);
