		...
		ZkUtil.setZkBiEventListener(btnDelete, Events.ON_CLICK, event -> {
         	boolean flag = false;
           	...
           	for (Object ts : selection) {
           		...
           		int lcrg = cc.getCellInt("lc_rg");
           		String lcdesc = cc.getCellString("lc_desc");
	   			TableRec tr = result.getSelectUtil().getQueryResult("select serial_id from metting where col_a = ?", new Wherecl().appendArgument(lcrg));
  		   		if (tr.getRecordCount() > 0) {
   		   			ZkBiMsgbox.show(ZkBiMsgbox.Type.error, String.format("產生過會議的物業'%s'不能被刪除", lcdesc));
   		   			flag = true;
   		   			break;
   		   		}
           	}
