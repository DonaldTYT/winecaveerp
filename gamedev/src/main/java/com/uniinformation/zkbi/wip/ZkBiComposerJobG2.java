...
public class ZkBiComposerJobG2 extends ZkBiComposerReport implements WipVisCallbackInterface  {
//	WfmJob wj;
...
//	WipVisNetwork	nW = null;
	...
	private Set<Integer> updatedJobRgList = new HashSet<Integer>();
...
//			        			biBaseRefreshListitems(br.getCurrentRecord());
            			...
            			updatedJobRgList.add(wed.getWfmrg());
			        if(currentIdx >= 0) {
			        	if(br.getCurrentRecIdx() != currentIdx) {
	...
	public void buildBrowserWindow(final BiResult result,final Component comp, int p_sortIdx, boolean p_sortDesc){
		...
		zkbiListTop = new Div();
		zkbiListTop.setId("zkbiListTop");
		zkbiListTop.setParent(comp);
		zkbiListPanel = new Div();
		zkbiListPanel.setId("zkbiListPanel");
		zkbiListPanel.setParent(zkbiListTop);
...
//							, eventDiv,"visTlContentDiv01");
	...
	protected void doZkbiItemSelected(int p_idx,BiResult p_br) {
		...
		if (wipTaskUpdate == null) { //andrew230912 fix null exception
			UniLog.log1("wipTaskUpdate is null, ignore");
			return;
		}
		try {
			currentIdx = p_idx;
    ...
    protected void setupExportPresetButton(final BiResult result) {
		super.setupExportPresetButton(result);
...
//								searchDiv.setVisible(false);
	...
	public void biBaseClose(BiResult p_br) {
		if (!updatedJobRgList.isEmpty()) {
       		for (int i = 0; i < br.getRowCount(); i++) {
            	br.loadOneRecV(i);
            	if (updatedJobRgList.contains(br.getCellInt("wfmj_rg"))) {
					br.fetchOneRecV(i);
					biBaseRefreshListitems(br.getCurrentRecord());
            		//br.reloadOneRecV(i);
            		//refreshListItems(br.getTrStatObj(i));			        			
