...
public class ZkBiComposerDO extends ZkBiComposerBase {
    ...
    protected void setupExtraButton(final BiResult result) {
    	...
    	if(masterWin.hasFellow("btCheckBalance")) {
    		btCheckPrice = (Button) masterWin.getFellow("btNonZero");
    	} 
    	else {	
    ...
    boolean checkDnPrice(BiResult result) {
    	...
    	for(BiCellCollection bc : rows) {
    		double soprice = bc.getCellDouble("ind_uprice") - bc.getCellDouble("ind_discount");
    		double dnprice = bc.getCellDouble("stmd_uprice");
    		if(soprice != dnprice ) return(false);
    	}
    	return(true);
    }
    boolean updateDnPrice(BiResult result) throws Exception {
    	...
    	for(BiCellCollection bc : rows) {
    		...
    		double dnprice = bc.getCellDouble("stmd_uprice");
    		if(soprice != dnprice ) {
    			bc.getCell("stmd_uprice").sync(soprice);
    		}
    	}
    	return(true);
    }
