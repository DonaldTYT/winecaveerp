...
public class ZkBiComposerStock extends ZkBiComposerBase {
	...
	protected void setupAddButton(final BiResult result)
	{
		Button btnScanRec;
		Button btScanE;
    	if(masterWin.hasFellow("btScanRec")) {
    		btnScanRec = (Button) masterWin.getFellow("btScanRec");
    	} 
    	else {	
	        btnScanRec = new ZkBiButton();
	        btnScanRec.setLabel("Scan");
	        btnScanRec.setId("btScanRec");
	        actionBar.appendChild(btnScanRec);
    	} 
        abHelper.addButton(btnScanRec);
    	if(masterWin.hasFellow("btScanE")) {
    		btScanE = (Button) masterWin.getFellow("bScanE");
    	} 
    	else {	
	        btScanE = new ZkBiButton();
	        btScanE.setLabel("Scanned");
	        btScanE.setId("btScanE");
	        btScanE.setVisible(false);
	        actionBar.appendChild(btScanE);
    	} 
        btnScanRec.addEventListener("onClick",
            new EventListener() {
        ...
        btScanE.addEventListener("onClick",
            new EventListener() {
               	public void onEvent(Event event) throws Exception {	
               		UniLog.log("scanner closed");
                };
        	}
        );
        Textbox txNewBarcode;
       	if(masterWin.hasFellow("txNewBarcode")) {
    		txNewBarcode = (Textbox) masterWin.getFellow("txNewBarcode");
    	} 
    	else {	
	        txNewBarcode = new Textbox();
	        txNewBarcode.setId("txNewBarcode");
	        actionBar.appendChild(txNewBarcode);
    	} 
        txNewBarcode.addEventListener(Events.ON_CHANGE,
            new EventListener() {
               	public void onEvent(Event event) throws Exception {	
               		UniLog.log("barcode changed");
               		String s = ((Textbox)event.getTarget()).getText();
    				...
    				try {
    					...
    					for(int i=0;i<result.getRowCount();i++) {
    						...
    						if(result.getCell("st_irg").getInt() == irg) {
    							found = true;
    					...
    					if(!found) {
    						...
    						ZkUtil.showMsg("Stock Not Available");	
    					}
    				} catch (Exception ex) {
...
//										} else {
//											DeviceControl.DevHandler dhdr = DeviceControl.getDevHandler("PTR01");
//											if(dhdr != null) {
...
////											dos.writeBytes("SIZE 70 mm, 40.0 mm\n");
...
//											dos.write(String.format("TEXT 50,300,\"ROMAN.TTF\",0,12,12,\"%s\"\n",escStr(result.getCellString("st_icode"))).getBytes("UTF-8"));
//											{
//												List<String> listStr = ChnftrParser.splitText(result.getCellString("st_iname"),"helv_nr","chinese",(float) 12.0,180);
//												UniLog.log(listStr.toString());
//												for(int i=0;i<3;i++) {
//													if(listStr.size() <= i) break;
//													String ss = listStr.get(i);
//													dos.write(String.format("TEXT 350,%d,\"DFFN.TTF\",0,12,12,\"%s\"\n",50+i*60,escStr(ss)).getBytes("UTF-8"));
//												}
//											}
