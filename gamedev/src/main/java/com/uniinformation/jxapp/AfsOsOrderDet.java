...
public class AfsOsOrderDet extends JxZkBiBase {
	class AfsOsOrderDetGetItemProperty extends BiGetItemProperty {
		AfsIcodePicker jxf = null;
		AfsOsOrderDetGetItemProperty(BiResult p_br) {
			super(p_br);
		}
		@Override
...
//						AfsIcodePicker jxf = (AfsIcodePicker) zjpi.getJxZkForm();
						...
						Wherecl wcl = null;
//						jxf.setPickerWithPO(AfsOsOrderDet.this,getBr().getSelectUtil(), 
//								new Wherecl().andUniop("st_icode", "=", getBr().getCell("st_icode").getString())
//								,bcc,bcc.getCollection().getCell("stmd_org"));
...
//						double reqQty = 0;
						...
						*/
						...
						jxf.setPickerForAvailableStock(AfsOsOrderDet.this,getBr().getSelectUtil(),wcl
								,bcc
...
//		LOCK_RECORD_FOR_UPDATE = true;
	...
	public void bindCellCollection(BiResult br,int mode) {
		boolean isNew;
		isNew = getGipi("AfsMoDet") == null;
//		if(getGipi("AfsMoDet") == null) {
		if(isNew) {
			setGipi("AfsOsDet",new AfsOsOrderDetGetItemProperty(br.getSubLink("AfsOsDet")));	
		}
		super.bindCellCollection(br, mode);
//		if(isNew) jxAdd("list_AfsMoDet").setAttribute("paging", "withfilter");
	}
