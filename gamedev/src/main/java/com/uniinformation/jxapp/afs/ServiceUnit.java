...
public class ServiceUnit extends JxZkBiBase {
		...
		try {
				...
				if(tr.getRecordCount() > 0) {
					...
					for(i=0;i< tr.getRecordCount();i++) {
						...
						switch(BiResultQuoDet.getDeltaType(getSessionHelper(),pdsrg)) {
...
//	}
	...
	void loadBundleItems() {
		JxField jf = jxAdd("bundlelist");
		if(jf == null) return;
		/*
		Listbox bundlelb = (Listbox) jf.getNativeObject();
		if(bundlelb == null) return;
		*/
		jf.clear();
		jf.gridSetCol(4);
		int setOdrg = getBr().getCellInt("svmc_odrg");
		if( setOdrg <= 0) return;
		int pdsrg = getBr().getCellInt("ind_pdsrg");
		if(pdsrg != 5) return;
		int setSeq = getBr().getCellInt("ind_seq");
		int indRg = getBr().getCellInt("ind_rg");
		SelectUtil su = getBr().getSelectUtil();
		try {
			TableRec tr = su.getQueryResult("select * from quodet,stock where ind_rg = ? and ind_seq > ? and st_irg = ind_irg order by ind_seq",
					new Wherecl().appendArgument(indRg).appendArgument(setSeq)
			);
			int r = 0;
			for(int i=0;i<tr.getRecordCount();i++) {
				tr.setRecPointer(i);
				if(tr.getFieldInt("ind_pdsrg") != 1) break;
				if(tr.getFieldString("st_mtype").equals("B")) {
					jf.gridSetRow(r+1);
					jf.gridSetValue(0, r, tr.getFieldString("st_iname"));
					jf.gridSetValue(1, r, tr.getFieldDouble("ind_qty"));
					jf.gridSetValue(2, r, tr.getFieldString("ind_unit"));
					jf.gridSetValue(3, r, "");
					r ++;
				}
			}
