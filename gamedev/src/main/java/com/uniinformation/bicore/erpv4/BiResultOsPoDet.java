...
public class BiResultOsPoDet extends BiResultErpv4 {
	public BiResultOsPoDet(BiResult p_parent, BiView p_view, SelectUtil p_su, Vector p_tabList, String p_whereStr,
	...
	protected HashSet<BiTable> addExtraWhereStr(Wherecl p_where,HashSet<BiTable> p_hash)
	{
		...
		if(locCol != null && columnInSelectList(locCol)) {
			...
			wcl1.appendString(" and " + "stmov_dp.stm_cocode = '"+cocode+"' ").stripAnd();
			...
			*/
			...
			if(locCol != null && columnInSelectList(locCol)) {
				String cocode = Erpv4Config.getDefaultCoCode(sh);
				Wherecl wcl1 = new Wherecl();
				wcl1.appendString(" and " + "locationcode.loc_cocode = '"+cocode+"' ").stripAnd();
				p_where.andWherecl(wcl1);
