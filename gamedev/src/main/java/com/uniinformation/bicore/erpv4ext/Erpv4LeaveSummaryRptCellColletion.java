...
import com.uniinformation.bicore.erpv4ext.BiResultLeaveSummaryRpt.LeaveCalExt;
...
public class Erpv4LeaveSummaryRptCellColletion extends BiCellCollection {
	...
	private Map<String, LeaveCalExt> getLeaveCalExtCacheMap() {
		if (getBr() instanceof BiResultLeaveSummaryRpt)
			return ((BiResultLeaveSummaryRpt)getBr()).getLeaveCalExtCacheMap();
		return null;
	}
//
...
//		double dd = NumberUtils.toDouble(BiResultLeaveApplication.getLeaveUnit2LvStr(d));
	...
	public Object evalFunction(String p_fname, Vector p_args) throws Exception {
		...
		switch (funcName){
			case FUNC_getNDays:
				...
				if(refKey.equals("emx_aldays")) {
					Map<String, LeaveCalExt> cacheMap = getLeaveCalExtCacheMap();
					if(cacheMap != null) {
						LeaveCalExt lvExt = cacheMap.get("eid");
						if(lvExt != null) {
							return(lvExt.aldays);
						}
					}
					
//				UniLog.log1("refKey:%s, sid:%d, eid:%s, cacheMap:%b, emstdate:%s, emenddate:%s, stalcnt:%d, maxalcnt:%d, ofsalcnt:%d, alstday:%s, alendday:%s", refKey, getSid(), eid, cacheMap != null, emstdate, emenddate, stalcnt, maxalcnt, ofsalcnt, alstday, alendday);
...
//				String alendday = (String)p_args.get(8);
...
//				LeaveCal leaveCal = cacheMap.get(eid);
//				if (leaveCal == null) {
//					leaveCal = new LeaveCal(br, emstdate, emenddate, stalcnt, maxalcnt, ofsalcnt, alstday, alendday);
...
//					//sbQuery.append(String.format(" and lv_edate >= '%s'", ddf.format(queryStDate)));
...
//							maxAlDate = (Date)ObjectUtils.max(maxAlDate, edate);
//							UniLog.log1("albal:%d, sdate:%s, leaveunit:%d, maxAlDate:%s", albal, sdate, leaveunit, maxAlDate);
//							if (edate.compareTo(queryStDate) >= 0) {
...
//					}
//					
//					if(queryEndDate.before(DateUtil.today())) {
//						maxAlDate = queryEndDate;
//					}
//					
//					//albal = leaveCal.genCalLeaveRemained("AL", queryEndDate, 0);
//					albal = leaveCal.genCalLeaveRemained("AL", maxAlDate, 0);
//					UniLog.log1("albal:%d, maxAlDate:%s", albal, maxAlDate);
