(function() {
  ...
  callWithJQuery(function($) {
    return $.pivotUtilities.zkbi_renderers = {
      "ZKBI": function(pivotData, opts) {
        ...
        var zkComp = null;
        var zkCompId = (opts.zkOpt).compId;
        if (zk !== null && zkCompId !== null) {
        	zkComp = zk.Widget.$(zkCompId);
        }
        else{
        	console.log('zkCompId is null');
        }
       	if (zAu !== null && zkComp !== null){
       		console.log('send event to ' + zkCompId);
    		zAu.send(new zk.Event(zkComp, "onPivotCallback", text, {toServer:true}));
       	}
       	else{
