...
var PDFViewerApplication = {
  ...
  viewerPrefs: {
	...
	disableViewBookmark: false,
    disableDownload: false,
    disablePrinting: false,
    disableCloseDialog: false
  },
  isViewerEmbedded: window.parent !== window,
  url: '',
  baseUrl: '',
  downloadFunctionName: null,
  closeDialogFunctionName: null,
  ...
  _readPreferences: function _readPreferences() {
    ...
    if ('scalevalue' in hashParams)
    	viewerPrefs['scaleValue'] = hashParams['scalevalue'];
    if ('disablefullscreen' in hashParams)
    	viewerPrefs['disableFullscreen'] = hashParams['disablefullscreen'] === 'true';
    if ('disableopenfile' in hashParams)
    	viewerPrefs['disableOpenFile'] = hashParams['disableopenfile'] === 'true';
    if ('disableviewbookmark' in hashParams)
    	viewerPrefs['disableViewBookmark'] = hashParams['disableviewbookmark'] === 'true';
    if ('disabledownload' in hashParams)
    	viewerPrefs['disableDownload'] = hashParams['disabledownload'] === 'true';
    if ('disableprinting' in hashParams)
    	viewerPrefs['disablePrinting'] = hashParams['disableprinting'] === 'true';
    if ('disableclosedialog' in hashParams)
    	viewerPrefs['disableCloseDialog'] = hashParams['disableclosedialog'] === 'true';
  ...
  get supportsPrinting() {
    return PDFPrintServiceFactory.instance.supportsPrinting && !this.viewerPrefs['disablePrinting'];
  },
  get supportsDownload() {
    return !this.viewerPrefs['disableDownload'];
  },
  get supportsCloseDialog() {
    return !this.viewerPrefs['disableCloseDialog'];
  },
  get supportsFullscreen() {
    ...
    if (support)
   		support = !this.viewerPrefs['disableFullscreen'];
    return (0, _pdfjsLib.shadow)(this, 'supportsFullscreen', support);
  },
  get supportsOpenFile() {
   	return !this.viewerPrefs['disableOpenFile'];
  },
  get supportsViewBookmark() {
   	return !this.viewerPrefs['disableViewBookmark'];
  },
  ...
  get defaultScaleValue() {
      return this.viewerPrefs['scaleValue'] ? this.viewerPrefs['scaleValue'] : _ui_utils.DEFAULT_SCALE_VALUE;
  },
  get loadingBar() {
  ...
  download: function download() {
	...
	if (this.downloadFunctionName) {
		try {
			eval("parent." + this.downloadFunctionName);
		} catch (exception) {
			alert(exception);
		}
	} else
		alert('download function no support');
  },
  closeDialog: function closeDialog() {
	  if (this.closeDialogFunctionName) {
		  try {
			  eval("parent." + this.closeDialogFunctionName);
		  } catch (exception) {
			  alert(exception);
		  }
	  } else
		  alert('closeDialog function no support');
  },
  ...
  setInitialView: function setInitialView(storedHash) {
    ...
    if (!this.pdfViewer.currentScaleValue) {
      this.pdfViewer.currentScaleValue = this.defaultScaleValue;
    }
  },
  ...
  bindEvents: function bindEvents() {
    ...
    eventBus.on('closedialog', webCloseDialog);
  ...
  unbindEvents: function unbindEvents() {
    ...
    eventBus.off('closedialog', webCloseDialog);
...
function webViewerInitialized() {
  ...
  var toolbuttons = {
	bookmark: {tbButton: appConfig.toolbar.viewBookmark, stbButton: appConfig.secondaryToolbar.viewBookmarkButton, support: PDFViewerApplication.supportsViewBookmark},
	download: {tbButton: appConfig.toolbar.download, stbButton: appConfig.secondaryToolbar.downloadButton, support: PDFViewerApplication.supportsDownload},
	print: {tbButton: appConfig.toolbar.print, stbButton: appConfig.secondaryToolbar.printButton, support: PDFViewerApplication.supportsPrinting},
	openFile: {tbButton: appConfig.toolbar.openFile, stbButton: appConfig.secondaryToolbar.openFileButton, support: PDFViewerApplication.supportsOpenFile},
	presentationModeButton: {tbButton: appConfig.toolbar.presentationModeButton, stbButton: appConfig.secondaryToolbar.presentationModeButton, support: PDFViewerApplication.supportsFullscreen},
	closeDialog: {tbButton: appConfig.toolbar.closeDialog, stbButton: appConfig.secondaryToolbar.closeDialogButton, support: PDFViewerApplication.supportsCloseDialog},
  };
  ...
  appConfig.secondaryToolbar.cursorSelectToolButton.setAttribute('hidden', 'true');
  appConfig.secondaryToolbar.cursorHandToolButton.setAttribute('hidden', 'true');
  appConfig.secondaryToolbar.documentPropertiesButton.setAttribute('hidden', 'true');
  if (!window.File || !window.FileReader || !window.FileList || !window.Blob) {
    toolbuttons['openFile'].support = false;
  } else {
  ...
  for (var key in toolbuttons) {
	  if (!toolbuttons[key].support) {
		  toolbuttons[key].tbButton.classList.add('hidden');
		  toolbuttons[key].stbButton.classList.add('hidden');
	  } else {
		  toolbuttons[key].tbButton.classList.remove('hiddenSmallView');
		  toolbuttons[key].tbButton.classList.remove('hiddenMediumView');
		  toolbuttons[key].tbButton.classList.remove('hiddenLargeView');
		  toolbuttons[key].stbButton.classList.remove('visibleSmallView');
		  toolbuttons[key].stbButton.classList.remove('visibleMediumView');
		  toolbuttons[key].stbButton.classList.remove('visibleLargeView');
		  if (pos < 2) {
			  toolbuttons[key].tbButton.classList.add('hiddenSmallView');
			  toolbuttons[key].stbButton.classList.add('visibleSmallView');
		  } else if (pos < 4) {
			  toolbuttons[key].tbButton.classList.add('hiddenMediumView');
			  toolbuttons[key].stbButton.classList.add('visibleMediumView');
		  } else {
			  toolbuttons[key].tbButton.classList.add('hiddenLargeView');
			  toolbuttons[key].stbButton.classList.add('visibleLargeView');
		  }
		  pos++;
	  }
  }
  appConfig.secondaryToolbar.toggleButton.classList.remove('visibleSmallView');
  appConfig.secondaryToolbar.toggleButton.classList.remove('visibleMediumView');
  appConfig.secondaryToolbar.toggleButton.classList.remove('visibleLargeView');
  appConfig.toolbar.scaleSelectContainer.classList.remove('hiddenSmallView');
  appConfig.toolbar.scaleSelectContainer.classList.remove('hiddenMediumView');
  appConfig.toolbar.scaleSelectContainer.classList.remove('hiddenLargeView');
  if (pos == 0)
	  appConfig.secondaryToolbar.toggleButton.classList.add('hidden');
  else if (pos < 3) {
	  appConfig.secondaryToolbar.toggleButton.classList.add('visibleSmallView');
	  appConfig.toolbar.scaleSelectContainer.classList.add('hiddenSmallView');
  } else if (pos < 5) {
	  appConfig.secondaryToolbar.toggleButton.classList.add('visibleMediumView');
	  appConfig.toolbar.scaleSelectContainer.classList.add('hiddenMediumView');
  } else {
	  appConfig.secondaryToolbar.toggleButton.classList.add('visibleLargeView');
	  appConfig.toolbar.scaleSelectContainer.classList.add('hiddenLargeView');
  }
...
function webCloseDialog() {
  PDFViewerApplication.closeDialog();
}
  ...
  if (cmd === 1 || cmd === 8 || cmd === 5 || cmd === 12) {
    switch (evt.keyCode) {
      ...
      case 96:
        if (!isViewerInPresentationMode) {
          setTimeout(function () {
            pdfViewer.currentScaleValue = PDFViewerApplication.defaultScaleValue;
          });
          handled = false;
        }
        break;
  ...
  _createClass(PDFLinkService, [{
    ...
    value: function setHash(hash) {
      ...
      if (hash.indexOf('=') >= 0) {
        ...
        if ('downloadfun' in params) {
        	PDFViewerApplication.downloadFunctionName = params.downloadfun;
        }
        if ('closedialogfun' in params) {
        	PDFViewerApplication.closeDialogFunctionName = params.closedialogfun;
        }
      } else {
  ...
  _createClass(PDFViewer, [{
    ...
    value: function scrollPageIntoView(params) {
      ...
      if (scale && scale !== this._currentScale) {
        ...
        this.currentScaleValue = PDFViewerApplication.defaultScaleValue;
      }
...
var SecondaryToolbar = function () {
  function SecondaryToolbar(options, mainContainer, eventBus) {
    ...
    this.buttons = [{
      ...
      element: options.closeDialogButton,
...
var Toolbar = function () {
  ...
  _createClass(Toolbar, [{
    ...
    value: function reset() {
      ...
      this.pagesCount = 0;
      this.pageScaleValue = PDFViewerApplication.defaultScaleValue;
    ...
    value: function _bindListeners() {
      ...
      items.closeDialog.addEventListener('click', function () {
        eventBus.dispatch('closedialog');
      });
...
function getViewerConfiguration() {
  return {
    ...
    secondaryToolbar: {
      ...
      closeDialogButton: document.getElementById('secondaryCloseDialog'),
